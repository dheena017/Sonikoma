...
- **The Render Loop Issue:** Because state is combined into a single hook returning a massive object, any state update to _any_ variable (e.g., a progress counter tick, a cursor coordinate update during canvas drag, or moving a slider in `AdvancedSettings`) creates a new `appLogic` object.
