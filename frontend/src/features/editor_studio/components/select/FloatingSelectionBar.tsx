...
interface FloatingSelectionBarProps {
  ...
  // Storyboard mode specific handlers
...
export function FloatingSelectionBar({
  ...
  React.useEffect(() => {
    ...
    const observeTarget = (el: Element) => {
      ...
      elementMutationObserver = new MutationObserver(() => {
        ...
        const nextHeight = isVisible
          ? Math.round(el.getBoundingClientRect().height)
          : 0;
        setAssetsHeight((prev) => (prev !== nextHeight ? nextHeight : prev));
      });
      elementMutationObserver.observe(el, {
        attributes: true,
  ...
  const isModalOpen = Boolean(
    showAutoCropModal || showBubbleModal || hasDomModal
  );
  const visible =
    ((totalCount > 0 && selectedCount > 0) || isAnyBusy) && !isModalOpen;
    ...
    (() => {
      if (isTimeline) {
        if (selectedPanelIds && selectedPanelIds.size > 0 && setPanels) {
          ...
          clearSelection?.();
          addNotification?.(
            `Duplicated ${selectedPanelIds.size} panels successfully!`,
            "success"
          );
        }
      } else {
    ...
    ((order: "asc" | "desc") => {
      if (isTimeline) {
        ...
        if (selectedScraped && setSelectedScraped) {
          ...
          addNotification?.(
            `Selection sorted in ${order}ending order`,
            "success"
          );
        }
      }
    ...
    ((pos: number) => {
      if (isTimeline) {
        ...
        if (selectedScraped && selectedScraped.length > 0 && setScrapedImages) {
          ...
          const itemsToMove = scrapedImages.filter((img) =>
            selectedScraped.includes(img)
          );
          const remaining = scrapedImages.filter(
          ...
          addNotification?.(
            `Moved ${selectedScraped.length} assets to position #${pos}`,
            "success"
          );
        }
      }
  ...
  return createPortal(
    <>
      ...
      {showMoreActions &&
        ...
        createPortal(
          <div
            style={{
              bottom: `${
                (typeof stackStyle.bottom === "number"
                  ? stackStyle.bottom
                  : parseInt(stackStyle.bottom || "0", 10) || 0) + barHeight
              }px`,
            }}
          ...
          >
            ...
            <div className="max-w-7xl mx-auto flex flex-wrap items-center gap-2">
              <div className="w-full flex items-center justify-between gap-2 mb-1.5 select-none">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[9px] text-neutral-400 font-bold font-mono px-2 py-0.5 bg-neutral-900 border border-neutral-800/80 rounded-lg shadow-inner">
                    {isTimeline ? "Storyboard" : "Imported Assets"}
                  </span>
                </div>
                <button
                  type="button"
              ...
              >
                <ClipboardCopy className="h-4 w-4 text-teal-400" /> Copy
                Metadata
              </button>

              <div className="flex items-center gap-1.5 bg-neutral-900 border border-neutral-800 rounded-xl px-2.5 py-1.5">
                <span className="text-[10px] text-neutral-450 font-mono font-medium">
                  Move to
                </span>
                <input
                  type="number"
                  ...
                  value={moveToPos}
                  onChange={(e) =>
                    setMoveToPos(Math.max(1, parseInt(e.target.value) || 1))
                  }
      ...
      <div
        id={
          isTimeline
            ? "timeline-selection-bar-portal"
            : "scraper-selection-bar-portal"
        }
      ...
      >
        ...
        >
          <div
            className={
              leftDock
                ? "h-full flex flex-col gap-3 overflow-auto custom-purple-scrollbar pb-1"
                : "bg-neutral-950/95 backdrop-blur-2xl border-t border-neutral-700/50 px-6 py-4 shadow-2xl shadow-black/60 w-full"
            }
          >
            <div
              className={
                leftDock
                  ? ""
                  : "max-w-7xl mx-auto flex items-center justify-between gap-3 flex-wrap md:flex-nowrap"
              }
            >
                ...
                {!isTimeline &&
                  ...
                  scrapedImages.length > 0 && (
                    <ScraperSelectionToolbar
                      ...
                      handleClearAll={
                        handleClearAll || (() => setSelectedScraped?.([]))
                      }
                  ...
                  onClick={
                    isTimeline
                      ? isAllSelected
                        ...
                        : selectAllPanels || handleSelectAllToggle
                      : handleSelectAllToggle
                  }
                  ...
                  <div className="flex items-center">
                    ...
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        e.preventDefault();
                        console.log(
                          "[FloatingSelectionBar] Opening AutoCrop modal"
                        );
...
// ── Storyboard selection bar ──────────────────────────

export function StoryboardSelectionBar(props: FloatingSelectionBarProps) {
  return <FloatingSelectionBar {...props} isTimeline={true} />;
}

export const TimelineSelectionBar = StoryboardSelectionBar;
