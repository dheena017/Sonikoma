...
export const AutoCropEngineSelector = React.memo(
  function AutoCropEngineSelector({
    useLocalCV,
    setUseLocalCV,
    cropModel,
    setCropModel,
    ...
    const [cvStrategyMode, setCvStrategyMode] = useState<
      "auto" | "grid" | "webtoon"
    >("auto");
    const [noiseThreshold, setNoiseThreshold] = useState<number>(5);
    const [bilateralBlur, setBilateralBlur] = useState<
      "none" | "soft" | "strong"
    >("soft");
    const [overlayTheme, setOverlayTheme] = useState<
      "cyan" | "emerald" | "violet" | "gold" | "rose"
    >("cyan");

    // AI Smart Engine Extended Controls State
    const [maxPanelCap, setMaxPanelCap] = useState<number>(0);
    const [confidenceStrictness, setConfidenceStrictness] =
      useState<number>(75);
    const [readingOrder, setReadingOrder] = useState<
      "manga" | "webtoon" | "western"
    >("manga");
    ...
    const testGeminiConnection = async () => {
      ...
      try {
        ...
        if (
          data &&
          (data.status === "healthy" || data.status === "ok" || hasKey)
        ) {
          setTestResult({
            success: true,
            latencyMs: latency,
    ...
    const promptHelpers = [
      {
        ...
        prompt:
          "Ignore top banner logos, watermarks, and series title artwork.",
      },
      {
      ...
      },
      {
        ...
        prompt:
          "Prioritize character illustration panels over background splash.",
      },
      {
        label: "Full Scene Merge",
        prompt:
          "Merge multi-segment vertical action scenes into single panels.",
      },
      {
      ...
      },
      {
        ...
        prompt:
          "Ignore header and footer page numbers, volume text, and chapter logos.",
      },
    ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <p className="text-[11px] text-neutral-400 leading-relaxed font-sans font-medium">
              Local Python edge and contour detection using Canny filtering.
              Works 100% offline with zero API limits.
            </p>
          </button>
          ...
          >
            ...
            <p className="text-[11px] text-neutral-400 leading-relaxed font-sans font-medium">
              Vision LLM panel segmentation. Understands complex comic layouts,
              overlapping speech balloons, and splash art.
            </p>
          </button>
        </div>
          ...
          >
            ...
            {setAspectRatioLock && (
              <div className="space-y-1.5">
                ...
                <div className="flex bg-neutral-900/60 p-1 rounded-xl border border-neutral-850 text-[9px] font-mono font-bold flex-wrap gap-1">
                  {["free", "1:1", "4:3", "16:9", "9:16", "2:3"].map(
                    (ratio) => (
                      <button
                        key={ratio}
                        type="button"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5 p-3 bg-neutral-900/30 border border-neutral-900 rounded-2xl">
                ...
                />
                <p className="text-[8px] text-neutral-500 leading-normal font-sans">
                  Discards small segments whose width ratio is below this
                  percent of full image.
                </p>
              </div>
              ...
              {setCropPaddingPx && (
                <div className="space-y-1.5 p-3 bg-neutral-900/30 border border-neutral-900 rounded-2xl">
                  ...
                  />
                  <p className="text-[8px] text-neutral-500 leading-normal font-sans">
                    Safety border padding added around detected panel crop
                    boxes.
                  </p>
                </div>
              )}
          ...
          >
            ...
            <div className="space-y-1.5 p-3.5 bg-neutral-900/30 border border-neutral-900 rounded-2xl">
              ...
              <input
                ...
                onChange={(e) =>
                  setConfidenceStrictness(Number(e.target.value))
                }
