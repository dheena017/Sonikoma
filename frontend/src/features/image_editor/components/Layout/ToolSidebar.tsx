...
import { useEditorState } from "@/features/image_editor/components/hooks/useEditorState";
import {
  Monitor,
  Scissors,
  Wand2,
...
export const ToolSidebar: React.FC = () => {
  const { activeTool, setActiveTool, isSidebarExpanded, setIsSidebarExpanded } =
    useEditorState();
    ...
    <div className="w-80 h-full bg-slate-900 border-l border-slate-800 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {activeTool === "workspace" && (
          <div className="text-slate-400">Workspace settings...</div>
        )}
        {activeTool === "autocrop" && (
          <div className="text-slate-400">Auto Crop controls...</div>
        )}
        {activeTool === "bubbleclean" && (
          <div className="text-slate-400">Bubble Cleaning controls...</div>
        )}
        {activeTool === "audiolab" && (
          <div className="text-slate-400">Audio Lab mixer...</div>
        )}
        {activeTool === "cuts" && (
          <div className="text-slate-400">Video cuts registry...</div>
        )}
      </div>
    </div>
  );
