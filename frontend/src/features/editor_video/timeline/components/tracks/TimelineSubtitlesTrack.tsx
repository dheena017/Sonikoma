...
export const TimelineSubtitlesTrack: React.FC<TimelineSubtitlesTrackProps> = ({
  ...
  const hasAnyText = panels.some(
    (p: any) => p.text_narration || p.caption || p.speech_text || p.narrative
  );
          ...
          panels.map((panel: any, idx: number) => {
            ...
            return (
              <div
                ...
                className={`group absolute flex items-center justify-between gap-1 select-none rounded-md border text-[9px] font-mono font-bold px-2.5 bg-[#2A2A2A] border-[#3B82F6]/40 text-[#3B82F6] z-10 ${
                  isMoving
