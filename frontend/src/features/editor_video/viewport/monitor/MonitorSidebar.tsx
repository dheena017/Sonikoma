...
const VideoPreviewSidebar: React.FC<VideoPreviewSidebarProps> = ({
  ...
  const episodeGroups =
    ((window as any).__scrapeEpisodeGroups as Array<any>) || [];
          ...
          {hasEpisodeGroups ? (
            getSortedEpisodeGroups(
              ...
              <button
                key={originalIdx}
                type="button"
                title={`Jump to ${formatDisplayEpisodeLabel(
                  grp.episodeLabel
                )} ΓÇö panel ${grp.startIndex + 1} ┬╖ ${grp.count} frames`}
                onClick={() => {
