      ...
      >
        <div
          className={`w-11 h-11 rounded-[1.1rem] flex items-center justify-center transition-all duration-300 shadow-sm ${
            isActive
              ? "bg-gradient-to-br from-purple-500/20 via-purple-600/20 to-indigo-600/20 border border-purple-500/50 shadow-[0_0_18px_rgba(168,85,247,0.28),inset_0_1px_0_rgba(255,255,255,0.12)]"
...
export const MiniSidebar: React.FC<MiniSidebarProps> = ({
  ...
  return (
    <aside className="w-16 sm:w-20 shrink-0 bg-[#09070f]/95 border-r border-purple-900/25 backdrop-blur-2xl shadow-[4px_0_34px_rgba(0,0,0,0.45)] flex flex-col items-center py-3.5 z-30 select-none h-full overflow-hidden">
      {/* Scrollable Nav */}
        ...
        {groupedWorkspaces.map((group, groupIdx) => (
          <div key={group.name} className="w-full flex flex-col items-center pb-1.5">
            ...
            >
              ...
              <span className="text-[8px] font-semibold uppercase tracking-[0.22em] text-neutral-400 font-mono select-none text-center w-full truncate whitespace-nowrap overflow-hidden px-1 py-0.5 bg-white/5 rounded-full">
                {group.name}
              </span>
            </div>
