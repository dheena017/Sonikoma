...
const VideoMultiTrackTimeline: React.FC<VideoMultiTrackTimelineProps> = ({
  ...
  const formatTimeShort = (secs: number) => `${secs % 60 === 0 && secs >= 60 ? Math.floor(secs / 60) + "m" : secs + "s"}`;
  ...
  // ── Compact track header (left label column) ──────────────────────────────
  const TrackLabel = ({
    id, label, color, type,
    ...
    <div className="w-full bg-[#111116] border-t border-white/[0.06] flex flex-col shrink-0 select-none h-[220px] z-20 font-sans">
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-h-0 relative" ref={trackAreaRef}>
        ...
        <div className="flex-1 overflow-y-auto [scrollbar-width:none]">
          ...
          {/* V1 — Main Video (image thumbnails) */}
          {!hiddenTracks["V1"] && (
          ...
          {/* ── Empty / drag-drop row (Canva style) ────────────────────────── */}
