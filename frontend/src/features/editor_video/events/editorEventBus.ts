...
class UnifiedEditorEventBus {
   ...
   */
  public subscribe<K extends EditorEventType>(
    event: K,
    ...
    return () => {
      this.listeners[event] = (
        this.listeners[event] as EventHandler<K>[] | undefined
      )?.filter((h) => h !== handler) as any;
    };
  }
   ...
   */
  public publish<K extends EditorEventType>(
    event: K,
    payload: EditorEventPayloads[K]
  ): void {
