...
export const RecentTemplateCard: React.FC<{
  item: RecentItem;
  onAction: () => void;
}> = ({ item, onAction }) => (
  <div
    onClick={onAction}
    className="p-3 rounded-[1.75rem] bg-[#07060f] border border-white/5 shadow-[0_18px_42px_rgba(0,0,0,0.18)] hover:border-amber-500/25 hover:shadow-[0_18px_48px_rgba(59,130,246,0.22)] transition-all duration-200 flex items-center justify-between gap-3 group cursor-pointer"
  >
    <div className="flex items-center gap-3 min-w-0 flex-1">
      <div className="h-11 w-11 rounded-3xl bg-amber-500/12 border border-amber-500/20 flex items-center justify-center shrink-0 shadow-inner group-hover:scale-105 transition-transform">
        <LayoutTemplate className="h-5.5 w-5.5 text-amber-300" />
      </div>
      <div className="min-w-0 flex-1 space-y-1">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[10px] text-neutral-400 font-mono">
          ...
          <span className="flex items-center gap-1 shrink-0">
            <Clock className="h-3 w-3 text-[#3B82F6]" /> {item.timeAgo}
          </span>
