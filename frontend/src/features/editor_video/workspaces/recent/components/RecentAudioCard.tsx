...
export const RecentAudioCard: React.FC<{ item: RecentItem; onAction: () => void }> = ({ item, onAction }) => (
  ...
  >
    <div className="flex items-center gap-3 min-w-0 flex-1">
      <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-violet-900/60 to-purple-950/90 border border-violet-500/40 flex items-center justify-center shrink-0 shadow-inner group-hover:scale-105 transition-transform">
        <Music className="h-4.5 w-4.5 text-violet-300" />
      </div>
      <div className="min-w-0 flex-1 space-y-1">
        <h4 className="text-xs font-bold text-white group-hover:text-purple-200 leading-tight font-sans truncate">
          {item.title}
        </h4>
