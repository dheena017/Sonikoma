  ...
  return (
    <WorkspaceLayout>
      ...
      <CharactersWorkspaceHeader
        ...
        onAddCharacter={() => onTriggerFeedback("New character prompt modal opened")}
            ...
            <div className="space-y-2">
              {MOCK_CHARACTERS.map((char) => (
                <CharacterRosterCard
                  key={char.id}
                  character={char}
                  onSelect={() => onTriggerFeedback(`Selected Character: ${char.name}`)}
                />
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {CHARACTER_EXPRESSIONS.map((exp) => (
                <CharacterExpressionCard
                  key={exp.id}
                  expression={exp}
                  onApply={() => onTriggerFeedback(`Applied expression: ${exp.name}`)}
                />
              ))}
            </div>
          </div>
            ...
            <div className="space-y-1.5">
              {CHARACTER_POSES.map((pose) => (
                <CharacterPoseCard
                  key={pose.id}
                  pose={pose}
                  onApply={() => onTriggerFeedback(`Applied pose: ${pose.title}`)}
                />
              ))}
            </div>
          </div>
