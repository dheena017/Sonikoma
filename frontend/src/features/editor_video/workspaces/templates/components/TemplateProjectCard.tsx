...
export const TemplateProjectCard: React.FC<TemplateProjectCardProps> = ({
  template,
  onApply,
}) => {
  return (
    <div
      className={`relative rounded-2xl overflow-hidden border cursor-pointer group transition-all ${template.accent}`}
    >
      <div
      ...
      <div className="relative z-10 p-4 space-y-2">
        ...
        <div className="flex items-center justify-between pt-1">
          <span
            className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded border ${template.accent}`}
          >
            {template.badge}
          </span>
          <button
