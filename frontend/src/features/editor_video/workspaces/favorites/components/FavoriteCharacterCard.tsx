...
export const FavoriteCharacterCard: React.FC<{
  ...
  >
    <div className="flex items-center gap-3 min-w-0 flex-1">
      {item.img ? (
        <img src={item.img} alt={item.title} className="h-10 w-10 rounded-xl object-cover shrink-0 border border-amber-500/40 shadow-inner group-hover:scale-105 transition-transform" />
      ) : (
        <div className="h-10 w-10 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center shrink-0 shadow-inner">
