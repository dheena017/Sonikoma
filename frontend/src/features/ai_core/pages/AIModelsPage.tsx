...
import { getTransactions, CreditTransaction } from "@/api/endpoints/auth";

import CredentialsAndTuner from "@/features/ai_core/components/CredentialsAndTuner";
import ModelRegistryExplorer from "@/features/ai_core/components/ModelRegistryExplorer";
import ModelPlaygroundAndSkills from "@/features/ai_core/components/ModelPlaygroundAndSkills";
import APITokenLedgerAndCosts from "@/features/ai_core/components/APITokenLedgerAndCosts";
import BenchmarkRunHistory from "@/features/ai_core/components/BenchmarkRunHistory";
...
const AIModelsPage = React.memo(
  ({
    ...
    const [runHistory, setRunHistory] = useState<any[]>(() => {
      try {
        return JSON.parse(
          localStorage.getItem("ai_playground_history") || "[]"
        );
      } catch {
        return [];
      }
      ...
      successfulRuns.length > 0
        ? Math.round(
            successfulRuns.reduce(
              (acc, curr) => acc + (curr.latencyMs || 0),
              0
            ) / successfulRuns.length
          )
        : 0;
    const totalCost = runHistory.reduce(
    ...
    const handleEnhancePrompt = async () => {
      ...
      try {
        ...
        if (data.success && data.enhanced_prompt) {
          ...
          throw new Error(
            data.error || "Prompt enhancement returned no result."
          );
        }
      } catch (err: any) {
        console.warn(
...
- Keep the response concise yet comprehensive.`;
    ...
    const exportRunHistory = () => {
      ...
      addNotification(
        "Exported benchmark run history successfully!",
        "success"
      );
    };
    ...
    return (
      <div className="flex-1 bg-neutral-955 text-neutral-100">
