    ...
    return (
      <div className="flex flex-col items-center justify-center p-16 space-y-4">
        ...
        <span className="text-sm font-medium text-neutral-400">
          Loading authentic backend diagnostics...
        </span>
      </div>
    );
  }
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="text-sm font-bold text-white tracking-wide">
                {server.service || "Sonikoma Backend"}
              </span>
            ...
            <p className="text-xs text-neutral-400 mt-0.5">
              {server.platform} • Python {server.python_version} • PID{" "}
              {server.process_id} • Port {server.backend_port}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-[#141414] border border-[#2F2F2F] rounded-2xl p-5 flex flex-col justify-between shadow-sm">
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-lg font-bold text-[#E5E5E5] font-mono">
              {cpu.usage_percent || 0}%
            </span>
          </div>
          ...
          <div className="flex justify-between text-[11px] text-[#9CA3AF] font-mono">
            ...
            <span>
              {cpu.cores_physical ? `${cpu.cores_physical} Physical` : "Active"}
            </span>
          </div>
        </div>
        ...
        <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-5 flex flex-col justify-between">
          ...
          <div className="flex justify-between text-[11px] text-neutral-400">
            <span>
              System: {Math.round(((memory.used_mb || 0) / 1024) * 10) / 10} GB
              Used
            </span>
            <span>
              {Math.round(((memory.total_mb || 0) / 1024) * 10) / 10} GB Total
            </span>
          </div>
        </div>
        ...
        <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-5 flex flex-col justify-between">
          ...
          <div className="space-y-1 text-[11px] text-neutral-300">
            <div className="flex justify-between">
              ...
              <span className="font-mono text-neutral-200">
                {database.engine || "SQLite"} ({database.journal_mode || "WAL"})
              </span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="flex justify-between text-[11px] text-neutral-400 pt-2 border-t border-neutral-800">
            <span>
              DB Size: {Math.round((database.file_size_bytes || 0) / 1024)} KB
            </span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-6">
          <div className="flex items-center justify-between mb-5">
            <h3 className="font-bold text-white flex items-center gap-2">
              <Database className="w-5 h-5 text-blue-400" /> Database Live Table
              Records
            </h3>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              {
                label: "Scrapes",
                count: counts.scrape_sessions,
                color: "text-rose-400",
              },
              {
                label: "Logs",
                count: counts.system_logs,
                color: "text-blue-400",
              },
              {
                label: "Token Logs",
                count: counts.token_usage_logs,
                color: "text-violet-400",
              },
              {
                label: "Transactions",
                count: counts.credit_transactions,
                color: "text-teal-400",
              },
              {
                label: "Settings",
                count: counts.platform_settings,
                color: "text-cyan-400",
              },
              {
                label: "Moderation",
              ...
              <div
                key={item.label}
                className="bg-[#0b0b0e] border border-neutral-800/80 rounded-lg p-3 text-center"
              >
                <div className={`text-xl font-bold font-mono ${item.color}`}>
                  {item.count ?? 0}
                </div>
                <div className="text-[11px] font-medium text-neutral-400 uppercase tracking-wider mt-1">
                  {item.label}
                </div>
              </div>
            ))}
        ...
        <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-6 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-white flex items-center gap-2">
                <Activity className="w-5 h-5 text-indigo-400" /> Background Job
                Queue
              </h3>
              <span className="text-[11px] font-mono text-neutral-400 bg-neutral-900 px-2 py-1 rounded border border-neutral-800">
            ...
            <div className="space-y-1.5">
              ...
              {jobQueue.recent_jobs && jobQueue.recent_jobs.length > 0 ? (
                jobQueue.recent_jobs.slice(0, 3).map((job: any) => (
                  <div
                    key={job.job_id}
                    className="flex items-center justify-between p-2 bg-[#0b0b0e] border border-neutral-800/80 rounded text-xs"
                  >
                    <div className="flex items-center gap-2 truncate">
                      <span className="font-mono text-neutral-400 text-[11px] truncate max-w-[120px]">
                        {job.job_id}
                      </span>
                ...
                <div className="text-xs text-neutral-500 italic p-3 text-center bg-[#0b0b0e] rounded">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-6">
          <h3 className="font-bold text-white mb-5 flex items-center gap-2">
            <Terminal className="w-5 h-5 text-emerald-400" /> Core Computational
            Engines
          </h3>
            ...
            {[
              {
                ...
                desc: capabilities.ffmpeg?.version || "System video rendering",
              },
              {
                name: "OpenCV (cv2)",
                cap: capabilities.opencv,
                desc: `Vision engine v${
                  capabilities.opencv?.version || "installed"
                }`,
              },
              {
                name: "PyTorch Deep Learning",
                cap: capabilities.torch,
                desc: `Inference runtime v${
                  capabilities.torch?.version || "installed"
                }`,
              },
              {
                name: "YOLO Segmentation",
                cap: capabilities.ultralytics,
                desc: capabilities.models?.yolov8n_seg?.exists
                  ? "yolov8n-seg.pt loaded"
                  : "Model ready",
              },
              {
                name: "EasyOCR Vision Model",
                cap: capabilities.easyocr,
                desc: "Speech bubble text reader",
              },
              {
                name: "Edge-TTS Speech Synth",
                cap: capabilities.edge_tts,
                desc: "Neural audio synthesis",
              },
              {
                name: "MoviePy Video FX",
                cap: capabilities.moviepy,
                desc: "Compositing timeline renderer",
              },
            ].map((tool) => (
              <div
                key={tool.name}
                className="flex items-center justify-between p-2.5 bg-[#0b0b0e] border border-neutral-800/80 rounded-lg"
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-neutral-500 truncate max-w-[280px]">
                      {tool.desc}
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-6">
            <h3 className="font-bold text-white mb-4 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-indigo-400" /> AI Provider API
              Keys
            </h3>
          ...
          <div className="bg-[#111115] border border-neutral-800/80 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-bold text-white flex items-center gap-2">
                <HardDrive className="w-5 h-5 text-amber-400" /> Storage
                Breakdown
              </h3>
              <span className="text-[11px] font-mono text-neutral-400 font-semibold">
                {(storage.total_app_storage_bytes || 0) < 1024 * 1024
                  ? `${Math.round(
                      (storage.total_app_storage_bytes || 0) / 1024
                    )} KB`
                  : `${
                      Math.round(
                        ((storage.total_app_storage_bytes || 0) /
                          (1024 * 1024)) *
                          10
                      ) / 10
                    } MB`}{" "}
            ...
            <div className="bg-[#0b0b0e] border border-neutral-800 p-3 rounded-lg mb-3">
              <div className="flex justify-between text-xs mb-1.5">
                ...
                <span className="font-mono text-neutral-200 font-semibold">
                  {Math.round(
                    ((partition.free_bytes || 0) / (1024 * 1024 * 1024)) * 10
                  ) / 10}{" "}
                  GB Free /{" "}
                  {Math.round(
                    ((partition.total_bytes || 0) / (1024 * 1024 * 1024)) * 10
                  ) / 10}{" "}
                  GB
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              {Object.entries(directories).map(([name, dir]: [string, any]) => (
                <div
                  key={name}
                  className="flex items-center justify-between p-2.5 bg-[#0b0b0e] border border-neutral-800/60 rounded-lg"
                >
                  <div className="min-w-0">
                    <div className="text-neutral-300 capitalize font-medium truncate">
                      {name.replace("_", " ")}
                    </div>
                    <div className="text-[10px] text-neutral-500 font-mono">
                      {dir.file_count || 0} file(s)
                    </div>
                  </div>
                  <span className="font-mono text-amber-400 font-semibold text-xs ml-2">
                    {dir.formatted_size ||
                      (dir.size_bytes
                        ? `${Math.round(dir.size_bytes / 1024)} KB`
                        : "0 B")}
                  </span>
                </div>
              ))}
