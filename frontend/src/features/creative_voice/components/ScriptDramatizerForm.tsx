      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 items-start">
        ...
        <div className="xl:col-span-5 space-y-4 bg-[#121212] border border-[#2F2F2F] p-4 rounded-xl shadow-inner">
          ...
          <div className="space-y-2 pt-1">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-mono text-[#60A5FA] uppercase tracking-widest block font-bold">
                DIALOGUE LINES TO DRAMATIZE ({rawLines.length} LINES)
              </label>
              <button
                type="button"
                onClick={() => setRawLines([...rawLines, ""])}
                className="text-[9px] font-mono text-[#3B82F6] hover:text-[#93C5FD] font-bold flex items-center gap-1 cursor-pointer bg-[#3B82F6]/10 px-2 py-0.5 rounded border border-[#3B82F6]/20"
              >
                + Add Line
              </button>
            </div>
            ...
            <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-purple-900/50">
              {rawLines.map((line, idx) => (
                <div
                  key={idx}
                  className="bg-neutral-900 border border-neutral-800 focus-within:border-[#3B82F6]/60 rounded-xl p-3 space-y-1.5 transition-all shadow-sm"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1.5">
                      <span className="text-[9px] font-mono font-bold text-[#60A5FA] bg-[#3B82F6]/20 border border-[#3B82F6]/30 px-1.5 py-0.5 rounded">
                        LINE #{idx + 1}
                      </span>
                      {panels[idx] && (
          ...
          <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-800 min-h-[360px] flex flex-col justify-between shadow-inner">
            {results.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center my-auto text-center py-12 text-neutral-500 space-y-3">
                <Sparkles className="h-9 w-9 text-[#3B82F6]/50 animate-pulse" />
                <div>
