...
import { Lock, Globe, ShieldCheck, CheckCircle2, ShieldAlert, Key, Copy, X } from "lucide-react";
...
interface ProfileSecurityTabProps {
  ...
  setPasswordState: React.Dispatch<React.SetStateAction<{
    current: string;
    new: string;
    confirm: string;
  }>>;
...
export default function ProfileSecurityTab({
  ...
  React.useEffect(() => {
    ...
    fetch(`/api/auth/audit-logs?query=${encodeURIComponent(searchQuery)}&page=${currentPage}&limit=3`, {
  ...
  const passwordStrength = React.useMemo(() => {
    ...
    if (score === 4) return { label: "Very Strong", percentage: 100, color: "bg-emerald-500" };
    if (score === 3) return { label: "Strong", percentage: 75, color: "bg-indigo-500" };
    if (score === 2) return { label: "Moderate", percentage: 50, color: "bg-purple-500" };
  ...
  const scoreLabel = () => {
    if (securityScore >= 95) return { grade: "A+", color: "text-emerald-400 bg-emerald-500/10 border-emerald-500/20" };
    if (securityScore >= 80) return { grade: "B", color: "text-purple-400 bg-purple-500/10 border-purple-500/20" };
    return { grade: "C", color: "text-amber-500 bg-amber-500/10 border-amber-500/20" };
  };
    ...
    <div className="space-y-6 animate-in fade-in duration-300 text-left">
      ...
      <div className="bg-[#0f0f13]/40 border border-white/5 rounded-3xl p-6 relative flex flex-col md:flex-row items-center justify-between gap-6 shadow-2xl">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h4 className="text-sm font-bold text-white">Security Integrity Score</h4>
            <p className="text-[10px] text-neutral-500 font-semibold mt-0.5">Automated workspace safety checklist review</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
          <div className="space-y-1">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-neutral-400 font-bold">2FA: {is2faEnabled ? "ACTIVE" : "INACTIVE"}</span>
            </div>
            <div className="text-[10px] text-neutral-500 font-semibold font-mono">Sessions Limit: {sessions.length} / 5 active</div>
          </div>

          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-xl border ${scoreLabel().color}`}>
            {scoreLabel().grade}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-[#0f0f13]/40 border border-white/5 rounded-3xl p-8 shadow-2xl relative">
          ...
          <div className="space-y-1 mb-6">
            ...
            <p className="text-xs text-neutral-400 font-semibold">Provide credentials to modify key security levels</p>
          </div>
                  ...
                  <div className="space-y-2 mt-2 bg-black/20 p-3 rounded-2xl border border-white/5 animate-in slide-in-from-top-1 duration-200">
                    <div className="flex justify-between items-center text-[10px]">
                      <span className="text-neutral-400 font-semibold">
                        Strength: <span className="font-bold text-white">{passwordStrength.label}</span>
                      </span>
                      <span className="text-neutral-500 font-mono font-bold">{passwordStrength.percentage}%</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[9px] font-semibold text-neutral-500 mt-1">
                      <div className="flex items-center gap-1">
                        <span className={pwRequirements.length ? "text-emerald-400" : "text-neutral-600"}>●</span>
                        <span className={pwRequirements.length ? "text-neutral-300" : ""}>Min 8 characters</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={pwRequirements.uppercase ? "text-emerald-400" : "text-neutral-600"}>●</span>
                        <span className={pwRequirements.uppercase ? "text-neutral-300" : ""}>Uppercase letter</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={pwRequirements.number ? "text-emerald-400" : "text-neutral-600"}>●</span>
                        <span className={pwRequirements.number ? "text-neutral-300" : ""}>Number (0-9)</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={pwRequirements.special ? "text-emerald-400" : "text-neutral-600"}>●</span>
                        <span className={pwRequirements.special ? "text-neutral-300" : ""}>Special symbol</span>
                      </div>
                    </div>
                  </div>
                )}
        ...
        <div className="bg-[#0f0f13]/40 border border-white/5 rounded-3xl p-8 shadow-2xl relative flex flex-col justify-between">
          ...
          <div className="space-y-1.5 mb-4">
            ...
            <p className="text-xs text-neutral-400 font-semibold">Secure your comic workspace uploads with secondary mobile auth keys</p>
          </div>
          ...
          {is2faEnabled ? (
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold rounded-2xl space-y-4">
              ...
              <p className="text-[10px] text-neutral-400 font-semibold leading-relaxed">Secondary biometric codes are required to perform exports and settings edits.</p>
              <button
            ...
            <div className="p-4 bg-neutral-900/40 border border-white/5 rounded-2xl space-y-4">
              ...
              <p className="text-[10px] text-neutral-500 leading-relaxed font-semibold">Enable MFA to protect developer API scopes and storyboard data caches.</p>
              ...
              {!show2faSetup ? (
                ...
                <form onSubmit={handleVerify2FA} className="space-y-4 pt-2 border-t border-white/5 animate-in slide-in-from-bottom-2 duration-300">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-1">
                      <span className="text-[9px] font-bold text-neutral-400 uppercase">Secret: AV_2FA_SEC_99A</span>
                      <p className="text-[9px] text-neutral-500 font-semibold">Scan with Google Authenticator or scan demo token code <strong>123456</strong></p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    />
                    {otpError && <p className="text-[9px] text-rose-400 font-bold ml-1">{otpError}</p>}
                  </div>
      ...
      <div className="bg-[#0f0f13]/40 border border-white/5 rounded-3xl p-8 shadow-2xl relative space-y-5">
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs text-neutral-400 font-semibold">View and manage sessions currently signed into your studio account</p>
        </div>
      ...
      <div className="bg-[#0f0f13]/40 border border-white/5 rounded-3xl p-8 shadow-2xl relative space-y-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-neutral-400 font-semibold">Track authentication events, profile changes, and API revisions</p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-white/5 text-xs text-neutral-300">
              {paginatedLogs.length === 0 ? (
                <tr>
                  <td colSpan={4} className="py-8 text-center text-neutral-500 font-semibold">
                ...
                paginatedLogs.map((log) => (
                  <tr key={log.id} className="hover:bg-white/5 transition-colors">
                    <td className="py-3 px-2 font-semibold text-white">{log.event}</td>
                    <td className="py-3 px-2 font-mono text-[10px] text-neutral-400">{log.ip}</td>
