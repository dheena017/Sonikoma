...
export function AutoCropPresetGrid({ activeSlot, applyPreset, firstImageUrl }: Props) {
  ...
  const presets = [
    { id: "standard", label: "⚖️ Standard Balanced", sub: "30% sens · 10px pad · Auto BG", config: { cropSensitivity: 30, cropPaddingPx: 10, cropBackgroundMode: "auto", aspectRatioLock: "free", autoSplitTallStrips: true, minPanelAreaPct: 1.5, overlapMergeThreshold: 30 } },
    { id: "webtoon",  label: "⚡ Webtoon Strip Slicer", sub: "25% sens · 15px pad · Fast split", config: { cropSensitivity: 25, cropPaddingPx: 15, cropBackgroundMode: "auto", aspectRatioLock: "free", autoSplitTallStrips: true, minPanelAreaPct: 1.0, overlapMergeThreshold: 45 } },
    { id: "manga",    label: "📖 Precise Manga", sub: "45% sens · 5px pad · White BG", config: { cropSensitivity: 45, cropPaddingPx: 5, cropBackgroundMode: "white", aspectRatioLock: "free", autoSplitTallStrips: false, minPanelAreaPct: 2.0, overlapMergeThreshold: 20 } },
      ...
      <div className="grid grid-cols-2 gap-3">
        {presets.map((p) => (
          <button key={p.id} type="button" onClick={() => applyPreset({ ...p.config, id: p.id })}
            className={`bg-neutral-950/40 border px-4 py-3.5 rounded-2xl text-left transition-all cursor-pointer ${border(p.id)}`}>
            <span className="text-[11px] font-bold text-white block">{p.label}</span>
            <span className="text-[9px] text-neutral-500 font-sans block mt-0.5">{p.sub}</span>
          </button>
        ))}
      </div>
    </div>
