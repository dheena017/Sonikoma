...
export function BubbleCleanerDiagnosticDashboard({ addNotification }: { addNotification?: (msg: string, type: any) => void }) {
  ...
  return (
    <div className={`border rounded-3xl p-5 space-y-3.5 transition-all duration-500 ${status === 'online' ? 'border-neutral-800 bg-neutral-950/20' : 'border-red-900 bg-red-950/10'}`}>
      <div className="flex items-center justify-between">
        <div className={`flex items-center gap-2 text-[10px] font-bold font-mono ${status === 'online' ? 'text-emerald-400' : 'text-red-400'}`}>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-2.5 rounded-xl bg-neutral-900/50 border border-neutral-800 text-[8px] font-mono">
          ...
          <span className={`font-bold ${status === 'online' ? 'text-emerald-400' : 'text-red-400'}`}>
            {status === 'online' ? 'STABLE (200 OK)' : status === 'checking' ? 'CHECKING...' : 'UNREACHABLE'}
          </span>
        </div>
        <div className="p-2.5 rounded-xl bg-neutral-900/50 border border-neutral-800 text-[8px] font-mono">
          <span className="text-neutral-500 block uppercase">LATENCY</span>
          <span className="text-indigo-400 font-bold">{latency ? `${latency}ms` : '--'}</span>
        </div>
        <div className="p-2.5 rounded-xl bg-neutral-900/50 border border-neutral-800 text-[8px] font-mono">
      ...
      <div className="flex items-start gap-3 bg-neutral-900/30 p-3 rounded-xl border border-neutral-800/50">
        ...
        <p className="text-[9px] text-neutral-500 font-sans leading-normal">
          Diagnostic logs indicate the Python bridge is {status === 'online' ? 'successfully' : 'not'} connected.
          {status === 'online'
