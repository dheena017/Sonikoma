...
export function GlobalScraperConfigTool({
  addNotification,
}: {
  ...
  const exportGlobalConfig = () => {
    ...
    try {
      ...
      const dataStr =
        ...
        encodeURIComponent(JSON.stringify(globalConfig, null, 2));
      ...
      a.remove();
      addNotification?.(
        "Global configuration exported successfully!",
        "success"
      );
    } catch (err: any) {
      addNotification?.(
        "Failed to export global config: " + err.message,
        "error"
      );
    }
  };
  ...
  const syncAllPresets = () => {
    ...
    try {
      ...
      addNotification?.(
        "Synced all Crop slots with Slot 1 parameters.",
        "success"
      );
    } catch (err) {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (config.presets) {
          if (config.presets.crop)
            localStorage.setItem(
              "crop_custom_presets",
              JSON.stringify(config.presets.crop)
            );
          if (config.presets.bubble)
            localStorage.setItem(
              "bubble_custom_presets",
              JSON.stringify(config.presets.bubble)
            );
    ...
    <div className="fixed bottom-6 right-6 z-[120]">
      ...
      >
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-neutral-900 border border-neutral-800 rounded-xl text-white text-[10px] font-bold hover:bg-neutral-800 shadow-2xl"
        >
          <Download className="h-3.5 w-3.5 text-indigo-400" />
          <span>Export Master Config</span>
        </button>
        <button
          onClick={syncAllPresets}
          className="flex items-center gap-2 px-4 py-2 bg-neutral-900 border border-neutral-800 rounded-xl text-white text-[10px] font-bold hover:bg-neutral-800 shadow-2xl"
        >
      ...
      <button
        ...
        className={`w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 shadow-[0_0_20px_rgba(99,102,241,0.3)] ${
          showTools
            ? "bg-indigo-600 rotate-90"
            : "bg-neutral-900 hover:bg-neutral-800"
        }`}
      >
        <Share2
          className={`h-5 w-5 ${showTools ? "text-white" : "text-indigo-400"}`}
        />
      </button>
    </div>
  );
