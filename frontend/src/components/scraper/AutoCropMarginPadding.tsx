...
export function AutoCropMarginPadding({ cropPaddingPx, setCropPaddingPx, firstImageUrl }: Props) {
    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between">
         ...
         <button
           ...
           className={`flex items-center gap-1 px-2 py-0.5 rounded-lg border text-[8px] font-bold transition-all ${smartPadding ? 'bg-violet-500/20 border-violet-500 text-violet-300' : 'bg-neutral-900 border-neutral-800 text-neutral-500'}`}
         >
            <Zap className={`h-2.5 w-2.5 ${smartPadding ? 'fill-current' : ''}`} />
            SMART PADDING
         </button>
      </div>
      <div className="bg-neutral-950/40 border border-neutral-800 rounded-xl p-4 space-y-3">
        ...
        <input
          type="range" min="0" max="40" value={cropPaddingPx}
          onChange={(e) => { setCropPaddingPx(Number(e.target.value)); setSmartPadding(false); }}
          className="w-full h-1 bg-neutral-900 rounded-lg appearance-none cursor-pointer accent-violet-500"
        />
        <p className="text-[8px] text-neutral-600 font-sans">Inserts additional gutter space around detected boundaries. Smart padding scales based on image resolution.</p>
      </div>
    </div>
  );
}
