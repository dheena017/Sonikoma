         ...
         {history.map((item, i) => (
            ...
            >
               <div className="flex flex-col">
                  <span className="text-[10px] text-neutral-300 font-bold">{item.name || `Session Config ${i+1}`}</span>
                  <span className="text-[8px] text-neutral-500 font-mono">
                    {item.cropSensitivity || item.sensitivity}% sens · {item.aspectRatioLock || item.detectionStyle}
                  </span>
               </div>
