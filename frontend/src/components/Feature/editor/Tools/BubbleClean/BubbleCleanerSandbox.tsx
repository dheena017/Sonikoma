...
export function BubbleCleanerSandbox({
  eraseMethod,
  addNotification,
  firstImageUrl,
}: Props) {
  ...
  const drawCartoonTemplate = (
    ...
    ctx.stroke();
    ctx.beginPath();
    ctx.arc(120, 100, 25, 0, Math.PI);
    ctx.stroke();
    ctx.fillStyle = "#ffffff";
    ctx.strokeStyle = "#000000";
    ctx.lineWidth = 2.5;
    ctx.beginPath();
    ctx.ellipse(130, 60, 50, 25, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    ctx.fillStyle = "#ffffff";
    ctx.beginPath();
    ...
    <div className="bg-neutral-950/40 border border-neutral-800 rounded-3xl p-5 space-y-4 flex flex-col items-center">
      <div className="w-full flex items-center justify-between text-[9px] font-mono">
        ...
        <div className="flex gap-2">
          <button
            onClick={undo}
            disabled={!canUndo}
            className="px-2 py-0.5 rounded border border-neutral-800 bg-neutral-900 text-neutral-400 disabled:opacity-30"
          >
            Undo
          </button>
          <button
      ...
      <div className="w-full space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <input
              ...
              className="w-20 h-1 bg-neutral-800 accent-purple-500"
            />
              ...
              {[
                ...
                <button
                  ...
                  className={`w-4 h-4 rounded-full border ${bg} ${
                    brushColor === color && brushMode === "paint"
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            type="button"
            onClick={() => simulateInpaint(eraseMethod)}
            className="py-1.5 rounded-lg border border-purple-800 bg-purple-950/20 hover:bg-purple-500/10 text-purple-300 text-[9px] font-bold font-mono transition-all active:scale-95 cursor-pointer"
          >
            Clean speech bubble
          </button>
          <button
            type="button"
            onClick={() => clearCanvas(drawCartoonTemplate)}
            className="py-1.5 rounded-lg border border-neutral-800 bg-neutral-900 hover:bg-neutral-800 text-neutral-400 text-[9px] font-bold font-mono transition-all active:scale-95 cursor-pointer"
          >
            Reset SandBox
          </button>
        </div>
