  ...
  const getRecommendation = () => {
    if (!imageSize)
      return {
        ...
        desc: "Select panels in the deck to auto-detect sizes.",
        style: "Free Ratio or Auto",
        warning: null,
      };
    ...
    if (ratio > 2.5) {
      return {
        ...
        warning: !autoSplit
          ? "Warning: Auto-Split is OFF. Tall strips might be too long for single frames."
          : null,
      };
    }
    if (ratio < 0.8)
      return {
        label: `Landscape Scene detected (${imageSize.width}x${imageSize.height}px)`,
        desc: `Wide strip layout (${(1 / ratio).toFixed(1)}:1).`,
        style: "16:9 Aspect Ratio Lock, Auto-Split OFF",
        warning: null,
      };
    return {
      label: `Standard Comic Page detected (${imageSize.width}x${imageSize.height}px)`,
      desc: "Standard manga page box.",
      style: "Free or 3:4 Aspect Ratio Lock",
      warning: null,
    };
  };
  ...
  return (
    <div className="lg:col-span-5 space-y-4">
      <div className="bg-neutral-950/60 border border-neutral-800 rounded-2xl p-4.5 space-y-2 font-mono relative overflow-hidden">
        ...
        <h4 className="text-[10px] font-bold text-white uppercase tracking-wider mt-1">
          {rec.label}
        </h4>
        <p className="text-[9px] text-neutral-500 leading-normal font-sans">
          {rec.desc}
        </p>
        <div className="text-[9px] bg-neutral-900 border border-neutral-800 px-2.5 py-1.5 rounded-lg text-emerald-400 font-semibold mt-1">
          {rec.style}
        </div>
        {rec.warning && (
      ...
      <div className="flex flex-col items-center justify-center p-4 bg-neutral-950/30 border border-neutral-800 rounded-2xl min-h-[180px]">
        <div className="w-full flex items-center justify-between mb-2.5">
          ...
          {manualSplits.length > 0 && (
            <button
              onClick={clearSplits}
              className="flex items-center gap-1 text-[7px] text-red-400 font-bold uppercase hover:text-red-300"
            >
        ...
        >
          ...
          {detectedPanels.map((p, i) => (
            <div
              key={i}
              className="absolute border border-emerald-400 bg-emerald-400/10 z-20 shadow-[0_0_4px_rgba(52,211,153,0.3)]"
              style={{
                top: `${p.cropTop}%`,
                left: `${p.cropLeft}%`,
            ...
            >
              <span className="absolute top-0 left-0 bg-emerald-500 text-black text-[5px] px-0.5 font-bold">
                {i + 1}
              </span>
            </div>
          ))}
          ...
          {!firstImageUrl && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-1 p-2 text-center select-none opacity-40">
              <div className="h-10 w-10 rounded-full border border-neutral-600 bg-neutral-800 flex items-center justify-center">
          ...
          <div
            className="absolute border-2 border-dashed border-white/20 transition-all duration-200 pointer-events-none"
            style={{
              top: `${cropPaddingPx / 2.5}%`,
              bottom: `${cropPaddingPx / 2.5}%`,
              left: `${cropPaddingPx / 2.5}%`,
              right: `${cropPaddingPx / 2.5}%`,
              boxShadow: `0 0 0 9999px rgba(0, 0, 0, ${
                0.4 + overlapMerge / 200
              })`,
              aspectRatio:
                aspectRatioLock === "1:1"
                  ? "1/1"
        ...
        <div className="flex flex-col items-center mt-2 space-y-1">
          ...
          {autoSplit && (
            <span className="text-[7px] text-indigo-400 font-bold uppercase tracking-tighter">
              Auto-Split algorithm active
            </span>
          )}
        </div>
      </div>
