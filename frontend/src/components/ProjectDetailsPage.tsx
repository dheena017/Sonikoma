    ...
    if (format === "json") {
      ...
      try {
        ...
        downloadAnchor.setAttribute(
          "download",
          `${(project.title || "storyboard").replace(
            /\s+/g,
            "_"
          )}_backup_${new Date().toISOString().slice(0, 10)}.json`
        );
                  ...
                  <div className="space-y-4">
                    ...
                    {filteredPanels.length === 0 ? (
                      <div className="py-12 text-center bg-[#0d0d10]/20 border border-white/5 rounded-3xl space-y-2">
                        ...
                        <p className="text-[10px] text-neutral-550 max-w-xs mx-auto leading-relaxed font-semibold">
                          Try searching for different dialogue keywords or
                          visual descriptions.
                        </p>
                      </div>
                    ) : (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                        {filteredPanels.map((panel) => {
                          const originalIdx = panels.findIndex(
                            (p) => p.id === panel.id
                          );
                          const isActive =
                            activePanelPreview &&
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="bg-[#0c0c10]/80 border border-white/5 rounded-3xl p-5 shadow-xl relative backdrop-blur-xl md:col-span-1 space-y-4">
                      ...
                      <div className="space-y-2.5">
                        ...
                        >
                          <span className="flex items-center gap-2">
                            <Sliders className="w-4 h-4 text-purple-450" />{" "}
                            Export Script Data (JSON)
                          </span>
                          <ChevronRight className="w-4 h-4 text-neutral-600 group-hover:text-purple-400" />
                        </button>
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-gray-500 mt-1">
                Total Duration: {totalDuration.toFixed(1)}s | Panels:{" "}
                {panels.length}
              </p>
            </div>
          </div>
              ...
              <div className="flex-1 space-y-3 text-left">
                <div className="flex justify-between items-center border-b border-gray-200 pb-1.5">
                  ...
                  <span className="text-[10px] text-gray-500 font-mono font-bold">
                    Duration: {panel.duration || 4.5}s | Motion:{" "}
                    {panel.motion_type || "zoom_in"}
                  </span>
                </div>
