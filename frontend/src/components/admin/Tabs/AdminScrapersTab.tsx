...
export function AdminScrapersTab({
  ...
  const fetchScraperSessions = async () => {
    ...
    try {
      const res = await fetchWithInterceptor(
        "/api/auth/admin/db/query?table=scrape_sessions&limit=20"
      );
      if (res.ok) {
  ...
  const handlePurgeProxyCache = async () => {
    try {
      ...
      addNotification("Error purging cache", "error");
    }
  };
    ...
    <div className="space-y-6 animate-[fadeIn_0.2s_ease-out]">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-[#111115] border border-neutral-800 rounded-xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="px-2 py-1 bg-emerald-500/10 text-emerald-400 text-[10px] font-bold rounded uppercase tracking-widest border border-emerald-500/20">
              Active
            </span>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center p-3 bg-[#0b0b0e] border border-neutral-800 rounded-lg">
              ...
              <div className="text-sm text-neutral-200 font-bold">
                Webtoons, MangaDex, Custom
              </div>
            </div>
            <div className="flex justify-between items-center p-3 bg-[#0b0b0e] border border-neutral-800 rounded-lg">
            ...
            <button
              onClick={handlePurgeProxyCache}
              className="w-full py-2 bg-neutral-800 hover:bg-neutral-700 text-white rounded-lg text-xs font-bold uppercase tracking-widest transition-all"
            >
      ...
      <div className="bg-[#111115] border border-neutral-800 rounded-xl overflow-hidden">
        <div className="p-4 border-b border-neutral-800 bg-[#0b0b0e] flex items-center justify-between">
          <h3 className="font-bold text-white flex items-center gap-2">
            <Database className="w-4 h-4 text-purple-400" /> Scraper Session
            History
          </h3>
          <button
              ...
              {sessions.map((s, i) => (
                <tr key={i} className="hover:bg-white/[0.02] group">
                  ...
                  <td className="px-6 py-4">
                    <div className="max-w-[300px] truncate text-neutral-300 font-medium">
                      {s.url}
                    </div>
                  </td>
