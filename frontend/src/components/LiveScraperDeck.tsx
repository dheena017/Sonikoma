...
export default function LiveScraperDeck({
  ...
  const [isBatchErasing, setIsBatchErasing] = React.useState<boolean>(false);
  const [batchEraseProgress, setBatchEraseProgress] = React.useState<{ current: number, total: number, method: 'inpaint' | 'blur' } | null>(null);
  ...
  const handleRemoveSpeechBubbles = async (imgUrl: string, method: 'inpaint' | 'blur') => {
    ...
    const activeMethod = associatedPanel?.bubble_method || method; 

    setConsoleLogs(prev => [
      `[Speech Bubbles] Removing text dialogue boxes using ${activeMethod} (sensitivity: ${sensitivity}%, dilation: ${dilation !== -1 ? dilation + "px" : "auto"}, radius: ${inpaint_radius}px, style: ${detection_style})...`,
      ...prev
    ]);
    ...
    try {
      ...
      if (data.success && data.url) {
        // Replace in scrapedImages
        setScrapedImages(prev => prev.map(img => img === imgUrl ? data.url : img));
        // Replace in selectedScraped
        setSelectedScraped(prev => prev.map(img => img === imgUrl ? data.url : img));
        // Replace in panels storyboard that are using this image
        setPanels(prev => prev.map(panel => panel.image_url === imgUrl ? { ...panel, image_url: data.url } : panel));

        const isBlur = method === "blur";
        let successMsg = "";
        if (data.bubbles_detected === false) {
          successMsg = "No distinct speech bubble candidates were detected, but processing finished successfully.";
        } else if (isBlur) {
          successMsg = "Blur Successful! The speech bubble text has been smudged together into a fuzzy, grayish-white blob.";
        } else {
          successMsg = "Inpaint Successful! The speech bubble vanishes, and the missing space is filled with matching background art.";
        }

        setConsoleLogs(prev => [
          `[Speech Bubbles] ${successMsg} (${isBlur ? "Smudge" : "Healing"} method)`,
          ...prev
        ]);
      ...
      console.error("[Speech Bubbles] Error:", err);
      addNotification(err.message || "Failed to remove speech bubbles.", "error");
      setConsoleLogs(prev => [
        `[Speech Bubbles ERROR] Bubble removal failed: ${err.message || err}`,
        ...prev
      ]);
      if (setErrorPopup) {
        setErrorPopup({
          ...
          onRetry: (overrideParams) => {
            setPanels(prev => prev.map(p => p.image_url === imgUrl ? {
              ...p,
            ...
            setTimeout(() => {
              handleRemoveSpeechBubbles(imgUrl, overrideParams.method as any);
            }, 100);
          }
        });
      }
  ...
  const handleBatchRemoveSpeechBubbles = async (method: 'inpaint' | 'blur') => {
    if (selectedScraped.length === 0) return;
    setIsBatchErasing(true);
    setConsoleLogs(prev => [
      `[Speech Bubbles Batch] Initiating batch bubble erasure using ${method === 'inpaint' ? 'Telea CV Inpaint' : 'Heavy Gaussian Blur'} for ${selectedScraped.length} selected frames...`,
      ...prev
    ]);
    ...
    try {
      ...
      setBatchEraseProgress({ current: 0, total: selectedScraped.length, method });

      for (let i = 0; i < selectedScraped.length; i++) {
        const imgUrl = selectedScraped[i];
        setBatchEraseProgress({ current: i + 1, total: selectedScraped.length, method });
        setCroppingImgUrl(imgUrl);
        ...
        if (!response.ok) {
          let errorMsg = `Bubble removal request failed with status ${response.status}`;
          try {
            const errData = await response.json();
            if (errData && errData.error) {
        ...
        if (data.success && data.url) {
          ...
          setPanels(prevPanels =>
            prevPanels.map(p => p.image_url === imgUrl ? { ...p, image_url: data.url, bubble_method: method } : p)
          );
      ...
      const isBlur = method === 'blur';
      const detailMsg = isBlur
        ? "All text has been smudged together into fuzzy, grayish-white blobs across selected panels!"
        : "Speech bubbles have vanished, and vacant spots have been filled with matching background art.";

      setConsoleLogs(prev => [
        `[Speech Bubbles Batch] Successfully completed bubble erasure (${isBlur ? "Blur/Smudge" : "Inpaint/Healing"}) for all ${selectedScraped.length} checked images!`,
        ...prev
      ]);
      addNotification(`Batch ${isBlur ? "Blur" : "Inpaint"} Completed Successfully! ${detailMsg}`, "success");
    } catch (err: any) {
      console.error("[Speech Bubbles Batch] process failed:", err);
      setConsoleLogs(prev => [
        `[Speech Bubbles Batch ERROR] Batch operation failed: ${err.message || err}`,
        ...prev
      ]);
      addNotification(err.message || "Batch speech bubble removal failed. Please try again.", "error");
      if (setErrorPopup) {
        setErrorPopup({
          title: "Batch Speech Bubble Eraser Interrupted",
          message: err.message || "During the batch sequence, one or more frame processing requests failed on the server. Batch execution stopped.",
          type: "error",
          technicalDetails: `Error Message: ${err.message || "Unknown"}\nBatch Size: ${selectedScraped.length} frames\nDefault Method: ${method}`,
          suggestion: "Please check your network connectivity, make sure the server is healthy, or process the specific frame individually to pinpoint which panel size/aspect ratio caused the error. You can then adjust settings and force-override.",
          parameters: {
            method: method,
            sensitivity: 50,
            dilation: -1,
          ...
          onRetry: (overrideParams) => {
            setTimeout(() => {
              handleBatchRemoveSpeechBubbles(overrideParams.method as any);
            }, 100);
          }
        });
      }
    } finally {
      setIsBatchErasing(false);
      setBatchEraseProgress(null);
      setCroppingImgUrl(null);
    }
  };
                    ...
                    {croppingImgUrl === imgUrl && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center z-20 bg-black/85 backdrop-blur-xs animate-fadeIn p-2 text-center select-none" id={`loading_overlay_${idx}`}>
                        ...
                        <span className="text-[9px] font-mono font-bold tracking-wider text-purple-300 uppercase animate-pulse">
                          {isBatchCropping 
                            ? "Auto-Cropping" 
                            : (isBatchErasing && batchEraseProgress?.method === "blur" ? "Selective Blur" : "CV Inpainting")
                          }
                        </span>
