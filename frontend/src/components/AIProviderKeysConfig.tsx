...
export default function AIProviderKeysConfig() {
  ...
  const handleClearAll = async () => {
    ...
    if (typeof (window as any).confirmAsync === "function") {
      confirmed = await (window as any).confirmAsync("Are you sure you want to delete all saved API keys?", "Clear Keys", "red");
    } else {
      confirmed = confirm("Are you sure you want to delete all saved API keys?");
    }
  ...
  const KeyInput = ({ 
    label, value, setter, provider, placeholder, isSaved, getLink, prefix
  }: { 
    label: string, value: string, setter: (v: string) => void, provider: string, placeholder: string, isSaved: boolean, getLink: string, prefix: string 
  }) => {
    ...
    <div className="flex flex-col gap-2 relative">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <a href={getLink} target="_blank" rel="noreferrer" className="text-[10px] text-purple-400 hover:text-purple-300 underline underline-offset-2 transition-colors">
            Get Key ↗
          </a>
          {isSaved && (
      ...
      <div className="relative group">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <Key className={`h-4 w-4 ${isSaved ? 'text-emerald-500' : isInvalid ? 'text-rose-500' : 'text-neutral-500 group-focus-within:text-purple-400'} transition-colors`} />
        </div>
        <input 
  ...
  return (
    <div className="bg-neutral-950/40 border border-neutral-850 rounded-3xl p-6 relative overflow-hidden group">
      ...
      <div className="flex items-start justify-between border-b border-neutral-800 pb-4 mb-5 relative">
        <div>
          ...
          <p className="text-[10px] text-neutral-500 font-mono mt-1">
            Keys are encrypted in transit and securely stored in your local browser storage.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 relative">
        <KeyInput label="Google Gemini" value={geminiKey} setter={setGeminiKey} provider="gemini" placeholder="e.g. AIzaSy..." isSaved={savedStatus.gemini} getLink="https://aistudio.google.com/app/apikey" prefix="AIza" />
        <KeyInput label="OpenAI" value={openAiKey} setter={setOpenAiKey} provider="openai" placeholder="e.g. sk-proj-..." isSaved={savedStatus.openai} getLink="https://platform.openai.com/api-keys" prefix="sk-" />
        <KeyInput label="Anthropic" value={anthropicKey} setter={setAnthropicKey} provider="anthropic" placeholder="e.g. sk-ant-..." isSaved={savedStatus.anthropic} getLink="https://console.anthropic.com/settings/keys" prefix="sk-ant" />
        <KeyInput label="Hugging Face" value={huggingFaceKey} setter={setHuggingFaceKey} provider="huggingface" placeholder="e.g. hf_..." isSaved={savedStatus.huggingface} getLink="https://huggingface.co/settings/tokens" prefix="hf_" />
      </div>
    </div>
  );
}
