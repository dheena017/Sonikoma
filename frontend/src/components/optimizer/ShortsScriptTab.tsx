...
export default function ShortsScriptTab({ title, storyboardSummary }: ShortsScriptTabProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-neutral-950/40 p-4 rounded-xl border border-neutral-800 flex justify-between items-center">
        <div>
          <h4 className="text-xs font-mono font-bold text-neutral-400 uppercase">Shorts & TikTok Video Retention Specialist</h4>
          <p className="text-[10px] text-neutral-500 font-mono mt-0.5">Generate fast voiceovers and scrolling hooks</p>
        </div>
        <button
      ...
      {loading && (
        <div className="bg-neutral-900/50 border border-neutral-800 rounded-xl p-8 text-center animate-pulse">
          ...
          <p className="text-[11px] font-mono text-purple-300 mt-2">Running Shorts Adapter & Hook Generator...</p>
        </div>
      )}
          ...
          {hookData && (
            <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-4 space-y-3 md:col-span-2">
              <div className="flex justify-between items-center border-b border-neutral-800 pb-2">
                <span className="text-[10px] font-mono font-bold text-purple-400">2-Second Scrolling Retention Hook</span>
                <button
              ...
              <div className="space-y-2">
                <p className="text-sm font-sans font-bold text-white italic">"{hookData.hook_sentence}"</p>
                <p className="text-[9px] font-mono text-neutral-500">
                  Psychology trigger focus: <span className="text-purple-300 font-semibold">{hookData.psychological_trigger}</span>
                </p>
              </div>
            </div>
          )}
          ...
          {scriptData && (
            <>
              <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-center border-b border-neutral-800 pb-2">
                  <span className="text-[10px] font-mono font-bold text-purple-400">55s Voiceover Script</span>
                  <button
                    onClick={() => copyToClipboard(scriptData.voiceover_script, "vo")}
                    className="text-neutral-500 hover:text-white p-1 rounded"
                  >
                    {copiedField === "vo" ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                  </button>
                </div>
              ...
              <div className="bg-neutral-900/40 border border-neutral-800 rounded-xl p-4 space-y-3">
                <div className="flex justify-between items-center border-b border-neutral-800 pb-2">
                  <span className="text-[10px] font-mono font-bold text-purple-400">Visual Milestones Timeline</span>
                  <button
                    onClick={() => copyToClipboard(scriptData.visual_milestones.join("\n"), "milestones")}
                    className="text-neutral-500 hover:text-white p-1 rounded"
                  >
                    {copiedField === "milestones" ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                  </button>
                </div>
                <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                  {scriptData.visual_milestones.map((ms, idx) => (
                    <div key={idx} className="flex gap-2 text-[10px] font-mono bg-neutral-950 p-2 rounded border border-neutral-850">
