...
export default function ShortsScriptTab({
  ...
  const handleGenerate = async () => {
    if (!videoUrl) {
      if (addNotification) {
        addNotification(
          "Please compile your video first on the Dashboard before generating Reels & Shorts content.",
          "warning"
        );
      }
      return;
    ...
    try {
      ...
      if (addNotification) {
        addNotification(
          "Successfully compiled Reels & Shorts specs!",
          "success"
        );
      }
    } catch (e) {
          ...
          <div className="bg-amber-950/20 border border-amber-900/40 rounded-xl p-4 flex gap-3 text-amber-250 animate-fade-in">
            ...
            <div className="space-y-1 text-xs">
              ...
              <p className="text-neutral-400 leading-relaxed font-sans">
                Reels & Shorts visual timelines and timing-dependent voiceovers
                cannot be generated accurately without the completed video
                duration. Please compile the video on the Dashboard first.
              </p>
            </div>
          </div>
        )}
