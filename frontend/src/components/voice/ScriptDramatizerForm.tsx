...
export default function ScriptDramatizerForm({ panels }: ScriptDramatizerFormProps) {
  const [loading, setLoading] = useState(false);
  const [genre, setGenre] = useState("Fantasy Action");
  const [context, setContext] = useState("The protagonist unlocks an ancient forbidden shadow power.");
  const [copiedIndex, setCopiedIndex] = useState<number | null>(null);
  const [results, setResults] = useState<string[]>([]);

  // Collect raw speech text from panels
  const initialRawLines = panels.length > 0
  ...
  const handleDramatize = async () => {
    ...
    try {
      ...
      if (json.success && json.result && json.result.dramatized_scripts) {
        ...
        const resList = Array.isArray(json.result) ? json.result : [json.result.dramatized_script || JSON.stringify(json.result)];
        setResults(resList);
      }
    } catch (e) {
      console.error(e);
    ...
    <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-5 space-y-4 shadow-xl">
      <div className="flex justify-between items-center border-b border-neutral-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-xs font-mono font-bold text-white uppercase">AI Dialogue Script Dramatizer</h4>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-3">
          <div className="space-y-1">
            <label className="text-[9px] font-mono text-neutral-500 uppercase">Genre Context</label>
            <input
              type="text"
              value={genre}
          ...
          <div className="space-y-1">
            <label className="text-[9px] font-mono text-neutral-500 uppercase">Scene Context / Plot details</label>
            <textarea
              rows={2}
              value={context}
          ...
          <div className="space-y-1">
            <label className="text-[9px] font-mono text-neutral-500 uppercase">Dialogue Lines to dramatize</label>
        ...
        <div className="space-y-3">
          <label className="text-[9px] font-mono text-neutral-500 uppercase">Dramatized Script Output</label>
            ...
            {results.length > 0 ? (
              <div className="space-y-3 overflow-y-auto max-h-60">
                {results.map((resLine, idx) => (
                  <div key={idx} className="group relative bg-neutral-900/50 p-2.5 rounded-lg border border-neutral-850/50">
                    <p className="text-xs text-neutral-200 pr-8 leading-relaxed font-sans">{resLine}</p>
                    <button
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-8 text-neutral-500 space-y-2">
                ...
                <p className="text-xs font-sans">No dramatized lines yet. Set the context and click "Enhance Script".</p>
              </div>
            )}
          </div>
        </div>
