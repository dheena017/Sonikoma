...
export default function ProfilePage({
  ...
  // Navigation tabs
  const [activeTab, setActiveTab] = React.useState<"projects" | "account" | "security" | "billing" | "api">("projects");
  ...
  const [profileUser, setProfileUser] = React.useState({
    ...
    bio: user?.bio || "Comic visual director and anime fan editing high-quality cinematic stories.",
  ...
  const [credits, setCredits] = React.useState(user?.credits !== undefined ? user.credits : 840);
  ...
  const [apiTokens, setApiTokens] = React.useState<{ id: string; name: string; key: string; created: string }[]>([]);
  ...
  React.useEffect(() => {
    ...
    .then(data => {
      if (data.user_id) {
        ...
        setConnections(data.social_connections || { google: true, github: false, discord: false });
        
        // Calculate achievement points based on unlocked rewards count
        setAchievementPoints(300 - (data.unlocked_rewards?.length || 0) * 100);

        setPortfolios((data.portfolio_links || []).map((url: string, idx: number) => ({
          id: idx.toString(),
          site: url.includes("webtoons") ? "Webtoons" : url.includes("tapas") ? "Tapas" : "ArtStation",
          url: url
        })));
        ...
        setProfileUser({
          ...
          bio: data.bio || "Comic visual director and anime fan editing high-quality cinematic stories.",
    ...
    .then(res => {
      if (res.success) {
        setApiTokens(res.keys.map((k: any) => ({
          id: k.key_id,
          name: k.name,
          key: k.api_key,
  ...
  const onRedeemReward = async (cost: number, type: string, value: string): Promise<boolean> => {
    const token = localStorage.getItem("anivox_token");
    if (!token) return false;
  ...
  const handleBatchDeleteProjects = (ids: string[]) => {
    ...
    .then(res => {
      if (res.success) {
        setLocalProjects(prev => prev.filter(p => !ids.includes(p.project_id)));
      }
    })
    .catch(err => {
      ...
      <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10">
        ...
        {isEditingAvatar && (
          <div className="p-6 bg-[#0f0f13]/80 border border-white/5 rounded-3xl animate-in slide-in-from-top-4 duration-300 space-y-4">
            <div className="text-left">
              <h4 className="text-sm font-bold text-white">Select Profile Avatar Theme</h4>
              <p className="text-xs text-neutral-500">Pick a curated comic color gradient avatar style</p>
            </div>
            <div className="flex flex-wrap gap-4">
              {AVATAR_TEMPLATES.map((gradient, idx) => (
                <button
                  key={idx}
                  onClick={() => {
                    setProfileUser(prev => ({ ...prev, avatarUrl: gradient }));
                    setIsEditingAvatar(false);
                  }}
                  className={`w-12 h-12 rounded-2xl cursor-pointer transition-transform hover:scale-105 active:scale-90 border-2 ${
                    profileUser.avatarUrl === gradient ? "border-purple-500 ring-2 ring-purple-500/30" : "border-transparent"
                  }`}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-neutral-900/40 border border-white/5 rounded-3xl p-5 text-left relative overflow-hidden shadow-xl">
            <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Total Compilation Queue</div>
          ...
          <div className="bg-neutral-900/40 border border-white/5 rounded-3xl p-5 text-left relative overflow-hidden shadow-xl">
            <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Average Panels per Strip</div>
            <div className="text-2xl font-black text-white mt-1">14 frames</div>
            <div className="text-[9px] text-indigo-400 font-semibold mt-1">CV bounding variance: optimal</div>
          </div>
          <div className="bg-neutral-900/40 border border-white/5 rounded-3xl p-5 text-left relative overflow-hidden shadow-xl">
            <div className="text-[10px] font-bold text-neutral-500 uppercase tracking-widest">AI Sync Success Rate</div>
            <div className="text-2xl font-black text-white mt-1">99.8%</div>
            <div className="text-[9px] text-emerald-400 font-semibold mt-1">Narration voice pitch matched</div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-1 space-y-4">
            ...
            <div className="bg-[#0c0c0e]/30 border border-white/5 rounded-3xl p-5 space-y-4 text-left">
              <h4 className="text-[10px] font-black uppercase text-neutral-500 tracking-widest">Workspace Stats</h4>
              <div className="space-y-3">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-[10px] text-neutral-400 font-semibold">
                    ...
                    <span className="text-white font-bold">{credits} / 1000</span>
                  </div>
                  <div className="h-1.5 bg-neutral-900 border border-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-purple-500 rounded-full" style={{ width: `${(credits/1000)*100}%` }} />
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between text-[10px] text-neutral-400 font-semibold">
                    ...
                    <span className="text-white font-bold">128.4 MB / 5 GB</span>
                  </div>
                  <div className="h-1.5 bg-neutral-900 border border-white/5 rounded-full overflow-hidden">
                    <div className="h-full bg-indigo-500 rounded-full" style={{ width: "2.5%" }} />
                  </div>
                </div>
