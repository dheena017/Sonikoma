...
const DashboardPage = React.memo(function DashboardPageInner() {
  ...
  const saveProjectName = useCallback(
    async (projectId: string, newName: string) => {
      ...
      // In a real application, you would make an API call here to update the project name.
      // For now, we'll just log it and reset the renaming state.
      console.log(`Renaming project ${projectId} to ${newName}`);

      // Optimistically update the UI if using client-side state, or refetch projects.
      // For this example, we'll just reset the renaming state.
      setRenamingProjectId(null);
    },
    []
  );
                  ...
                  .map((project) => {
                    ...
                    return (
                      <ProjectCard
                        ...
                        onRename={(e, projectItem) => handleRename(e, projectItem)}
                        onExport={(e, projectItem) => handleExport(e, projectItem)}
                        onDelete={(e, projectId) => handleDeleteProject(e, projectId)}
                        openMenuId={openMenuId}
                        onToggleMenu={(e, projectId) => toggleMenu(e, projectId)}
