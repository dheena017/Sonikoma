...
export default function RegisterPage({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null);
    try {
      await onRegister({ email, password, full_name: fullName, creator_role: creatorRole, subscribe_newsletter: subscribeNewsletter });
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-black/40 border rounded-xl py-3 pl-11 pr-4 text-sm text-white placeholder:text-neutral-700 focus:outline-none focus:ring-2 focus:ring-purple-600/20 transition-all font-medium ${
                      isEmailValid ? "border-emerald-500/20 focus:border-emerald-500/40" : "border-white/5 focus:border-purple-500/50"
                    }`}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className={`w-full bg-black/40 border rounded-xl py-3 pl-11 pr-10 text-sm text-white placeholder:text-neutral-700 focus:outline-none focus:ring-2 transition-all font-medium ${
                      hasMinLength ? "border-emerald-500/20 focus:border-emerald-500/40" : "border-white/5 focus:border-purple-500/50"
                    }`}
                    placeholder="••••••••"
                  />
                  <button
              ...
              {password.length > 0 && (
                <div className="space-y-2 px-1">
                  ...
                  <div className="flex items-center justify-between text-[10px] font-bold text-neutral-500 uppercase tracking-wider">
                    ...
                    <span className={passwordStrength > 0 ? strengthColor().replace("bg-", "text-") : "text-neutral-500"}>
                      {strengthText()}
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-1 pt-1">
                    <div className="flex items-center gap-1.5 text-[10px]">
                      <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all ${
                        hasMinLength ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-400" : "bg-white/5 border-white/5 text-neutral-600"
                      }`}>
                        <Check className="w-2.5 h-2.5 stroke-[4px]" />
                      </div>
                      <span className={hasMinLength ? "text-neutral-300" : "text-neutral-500"}>8+ characters</span>
                    </div>

                    <div className="flex items-center gap-1.5 text-[10px]">
                      <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all ${
                        hasUppercase ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-400" : "bg-white/5 border-white/5 text-neutral-600"
                      }`}>
                        <Check className="w-2.5 h-2.5 stroke-[4px]" />
                      </div>
                      <span className={hasUppercase ? "text-neutral-300" : "text-neutral-500"}>1 uppercase letter</span>
                    </div>

                    <div className="flex items-center gap-1.5 text-[10px]">
                      <div className={`w-3.5 h-3.5 rounded-full border flex items-center justify-center transition-all ${
                        hasNumber ? "bg-emerald-500/20 border-emerald-500/40 text-emerald-400" : "bg-white/5 border-white/5 text-neutral-600"
                      }`}>
                        <Check className="w-2.5 h-2.5 stroke-[4px]" />
                      </div>
                      <span className={hasNumber ? "text-neutral-300" : "text-neutral-500"}>1 number</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start ml-1 pt-1">
                <label className="flex items-start gap-2.5 cursor-pointer group select-none">
                  ...
                  <span className="text-xs text-neutral-400 group-hover:text-neutral-300 transition-colors font-medium leading-relaxed">
                    I accept Anivox's{" "}
                    <button type="button" className="text-purple-400 hover:text-purple-300 underline font-semibold">Terms of Service</button>
                    {" "}and{" "}
                    <button type="button" className="text-purple-400 hover:text-purple-300 underline font-semibold">Privacy Policy</button>.
                  </span>
                </label>
              </div>
