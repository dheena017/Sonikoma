...
export default function AdminPage({
  ...
  const [stats, setStats] = useState({ users: 0, projects: 0, scenes: 0, memory: "0MB" });
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div className="space-y-6">
          <div>
            ...
            <p className="text-neutral-400 mt-1">System overview and bulk user management.</p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#111115] border border-neutral-800 rounded-lg p-4 flex items-center gap-4">
              <div className="p-2.5 bg-blue-500/10 rounded-md border border-blue-500/20 text-blue-400">
            ...
            <div className="bg-[#111115] border border-neutral-800 rounded-lg p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs text-neutral-500 uppercase font-semibold">Projects</p>
                <p className="text-2xl font-bold text-white">{stats.projects}</p>
              </div>
            </div>
            
            <div className="bg-[#111115] border border-neutral-800 rounded-lg p-4 flex items-center gap-4">
              <div className="p-2.5 bg-pink-500/10 rounded-md border border-pink-500/20 text-pink-400">
                <Film className="w-5 h-5" />
              </div>
              <div>
                <p className="text-xs text-neutral-500 uppercase font-semibold">Scenes</p>
                <p className="text-2xl font-bold text-white">{stats.scenes}</p>
              </div>
            </div>

            <div className="bg-[#111115] border border-neutral-800 rounded-lg p-4 flex items-center gap-4">
              <div className="p-2.5 bg-emerald-500/10 rounded-md border border-emerald-500/20 text-emerald-400">
                <Activity className="w-5 h-5" />
              </div>
              <div>
                <p className="text-xs text-neutral-500 uppercase font-semibold">Sys Memory</p>
                <p className="text-2xl font-bold text-white">{stats.memory}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 bg-[#111115] border border-neutral-800 rounded-xl p-4">
          <div className="relative w-full sm:w-96">
            ...
            <input 
              ...
              className="w-full bg-[#0b0b0e] border border-neutral-800 text-sm text-neutral-200 rounded-lg pl-9 pr-4 py-2 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/50 transition-all"
            />
          </div>
                  ...
                  filteredUsers.map((u) => (
                    <tr key={u.id} className={`hover:bg-white/[0.02] transition-colors group ${selectedUsers.has(u.id) ? 'bg-purple-500/5' : ''}`}>
                      <td className="px-4 py-4">
                        <button onClick={() => toggleSelectUser(u.id)} className="text-neutral-500 hover:text-white transition-colors">
                      ...
                      <td className="px-4 py-4 text-neutral-500 font-mono text-xs flex items-center gap-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
          <div className="bg-[#111115] border border-neutral-800 rounded-xl w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
            <div className="p-6 border-b border-neutral-800 flex justify-between items-center">
              ...
              <button onClick={() => setViewLogsUser(null)} className="text-neutral-500 hover:text-white">
                ✕
              </button>
            </div>
