...
export default function CommentReplier({ title }: CommentReplierProps) {
  ...
  const [comment, setComment] = useState("This MC is so overpowered! When is the next episode coming out?");
    ...
    <div className="bg-neutral-900/40 border border-neutral-800 rounded-2xl p-5 space-y-4 max-w-xl mx-auto shadow-xl">
      <div className="flex justify-between items-center border-b border-neutral-800 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-xs font-mono font-bold text-white uppercase">AI Channel Subscriber Reply Generator</h4>
        </div>
        <button
      ...
      <div className="space-y-1">
        <label className="text-[9px] font-mono text-neutral-500 uppercase">Subscriber Comment</label>
        <textarea
          rows={3}
          value={comment}
      ...
      {reply && !loading && (
        <div className="bg-neutral-950 p-4 rounded-xl border border-neutral-850 space-y-3 animate-fade-in">
          <div className="flex justify-between items-center border-b border-neutral-900 pb-1.5">
            <span className="text-[9px] font-mono text-neutral-500 uppercase">Draft reply:</span>
            <button
              onClick={() => copyToClipboard(reply.reply_text, "reply")}
              className="text-neutral-500 hover:text-white"
            >
              {copiedField === "reply" ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
            </button>
          </div>
          ...
          <div className="pt-2 border-t border-neutral-900 text-[10px] font-mono text-neutral-500">
            <span className="text-purple-355 block font-bold text-[8px] uppercase tracking-wider">Tactic reasoning:</span>
