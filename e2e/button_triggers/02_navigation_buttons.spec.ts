...
test.describe("Button Triggers Suite > 02. Navigation & Modal Triggers", () => {
  test("UI-NAV-BTNS-001: Header navigation buttons are visible and clickable", async ({
    page,
  }) => {
  ...
  test("UI-NAV-THEME-002: Theme switch button or settings icon is responsive", async ({
    page,
  }) => {
    await page.goto("http://localhost:3000").catch(() => {});
    const themeBtn = page
      .locator(
        'button[title*="Theme"], button[title*="Dark"], button[title*="Light"], button'
      )
      .first();
