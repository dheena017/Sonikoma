...
test.describe("Button Triggers Suite > 06. System Admin & Health Controls", () => {
  test("UI-ADMIN-HEALTH-001: Health check button queries backend status", async ({
    page,
  }) => {
    await page.goto("http://127.0.0.1:5173/tests").catch(() => {});
    const probeBtn = page
      .locator("button")
      .filter({ hasText: /Re-Probe|Probing/i })
      .first();
  ...
  test("UI-ADMIN-SUITE-FILTER-002: Direct suite filter buttons switch active catalog", async ({
    page,
  }) => {
    await page.goto("http://127.0.0.1:5173/tests").catch(() => {});
    const filterBtn = page
      .locator("button")
      .filter({ hasText: /Endpoints|Functions|Buttons/i })
      .first();
