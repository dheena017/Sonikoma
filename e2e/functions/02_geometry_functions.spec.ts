...
test.describe("Functions Suite > Image Geometry & Panel Calculations", () => {
  test("IMAGE-PANEL-001: 16:9 Aspect Ratio Calculator produces exact dimensions", async ({
    page,
  }) => {
    const calc16by9 = (w) => ({ width: w, height: Math.round((w * 9) / 16) });
    const input = { width: 1920 };
    const output = calc16by9(1920);
    await renderTraceCard(page, "IMAGE-PANEL-001", {
      type: "Unit Function",
      target: "calc16by9()",
      status: "PASSED",
      input,
  ...
  test("IMAGE-PANEL-002: Bounding box area calculation", async ({ page }) => {
    ...
    await renderTraceCard(page, "IMAGE-PANEL-002", {
      type: "Unit Function",
      target: "calcArea()",
      status: "PASSED",
      input,
