...
test.describe("Functions Suite > Text & Dialogue Parsing Utilities", () => {
  test("TEXT-DIALOGUE-001: Extracts speaker label and cleaned dialogue line", async ({
    page,
  }) => {
    ...
    await renderTraceCard(page, "TEXT-DIALOGUE-001", {
      type: "Unit Function",
      target: "parseSpeaker()",
      status: "PASSED",
      input,
  ...
  test("TEXT-DIALOGUE-002: Truncates long panel captions with ellipsis", async ({
    page,
  }) => {
    const truncateCaption = (t, max = 30) =>
    ...
    await renderTraceCard(page, "TEXT-DIALOGUE-002", {
      type: "Unit Function",
      target: "truncateCaption()",
      status: "PASSED",
      input,
      output,
    });
