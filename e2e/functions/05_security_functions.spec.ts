...
test.describe("Functions Suite > Security & SSRF Protection", () => {
  test("SECURITY-SSRF-001: Blocks localhost loopback IPs", async ({ page }) => {
    ...
    await renderTraceCard(page, "SECURITY-SSRF-001", {
      type: "Unit Function",
      target: "isSafe()",
      status: "PASSED",
      input,
  ...
  test("SECURITY-SSRF-002: Blocks cloud metadata IP 169.254.169.254", async ({
    page,
  }) => {
    const isMetadata = (ip) => ip === "169.254.169.254";
    const input = "169.254.169.254";
    const output = isMetadata(input);
    await renderTraceCard(page, "SECURITY-SSRF-002", {
      type: "Unit Function",
      target: "isMetadata()",
      status: "PASSED",
      input,
