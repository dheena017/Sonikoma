...
test.describe("Endpoints Suite > 14. Image Proxy & Anti-Hotlinking", () => {
  test("API-PROXY-001: GET /api/v1/proxy/image parameter guard on empty url", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/proxy/image",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
  ...
  test("API-PROXY-002: GET /api/v1/proxy/stats proxy cache metrics", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/proxy/stats",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
