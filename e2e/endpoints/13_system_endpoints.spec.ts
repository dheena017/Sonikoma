...
test.describe("Endpoints Suite > 13. System Health & Diagnostics", () => {
  test("API-SYSTEM-001: GET /api/v1/health diagnostic status returns system state", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
  ...
  test("API-SYSTEM-002: GET /api/v1/system/status hardware & memory metrics", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/system/status",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
