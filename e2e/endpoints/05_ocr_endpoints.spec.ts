...
test.describe("Endpoints Suite > 05. OCR & Speech Extraction", () => {
  test("API-OCR-001: POST /api/v1/ocr/extract missing image handling", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/ocr/extract",
        { data: {}, timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
  ...
  test("API-OCR-002: GET /api/v1/ocr/languages supported OCR model languages", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/ocr/languages",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
