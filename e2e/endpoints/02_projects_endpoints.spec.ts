...
test.describe("Endpoints Suite > 02. Projects & Workspace", () => {
  test("API-PROJECTS-001: GET /api/v1/projects workspace listing", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
  ...
  test("API-PROJECTS-002: POST /api/v1/projects create project schema validation", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
  ...
  test("API-PROJECTS-003: GET /api/v1/projects/public/demo-project public viewer", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/projects/public/demo-project",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
      ...
      body = JSON.stringify({
        status: "healthy",
        route: "/api/v1/projects/public/demo-project",
        mode: "offline_guard",
      });
    }
