...
test.describe("Endpoints Suite > 08. Image Detection & Panels", () => {
  test("API-IMAGES-001: POST /api/v1/images/detect contract rejection on empty body", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/images/detect",
        { data: {}, timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
  ...
  test("API-IMAGES-002: POST /api/v1/images/crop bounding box validation", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/images/crop",
        { data: { box: [0, 0, 100, 100] }, timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
    } catch (e) {
