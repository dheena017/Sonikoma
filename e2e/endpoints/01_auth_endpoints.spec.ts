...
test.describe("Endpoints Suite > 01. Authentication & Security", () => {
  test("API-AUTH-001: POST /api/v1/auth/login invalid credentials rejection", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        ...
        {
          data: { email: "invalid@sonikoma.ai", password: "wrong" },
          timeout: 4000,
        }
      );
      status = res.status();
  ...
  test("API-AUTH-002: POST /api/v1/auth/token OAuth2 token form contract", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/auth/token",
        { data: { username: "test", password: "password" }, timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
    } catch (e) {
  ...
  test("API-AUTH-003: GET /api/v1/auth/me unauthorized session guard", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
