...
test.describe("Endpoints Suite > 12. Export & ZIP Packaging", () => {
  test("API-EXPORT-001: GET /api/v1/export/zip validates project_id parameter", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/export/zip",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
  ...
  test("API-EXPORT-002: GET /api/v1/export/mp4 validates render job identifier", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/export/mp4",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
