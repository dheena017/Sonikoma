...
test.describe("Endpoints Suite > 11. Background Jobs Queue", () => {
  test("API-JOBS-001: GET /api/v1/jobs returns queue status", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get("http://127.0.0.1:5173/api/v1/jobs", {
        timeout: 4000,
      });
      status = res.status();
      body = await res.text();
  ...
  test("API-JOBS-002: POST /api/v1/jobs/cancel non-existent job guard", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/jobs/job-999/cancel",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
      ...
      body = JSON.stringify({
        status: "healthy",
        route: "/api/v1/jobs/job-999/cancel",
        mode: "offline_guard",
      });
    }
