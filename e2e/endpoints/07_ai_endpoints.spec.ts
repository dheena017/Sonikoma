...
test.describe("Endpoints Suite > 07. AI Model Catalog", () => {
  test("API-AI-001: GET /api/v1/ai/models returns available models list", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/ai/models",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
  ...
  test("API-AI-002: GET /api/v1/ai/providers returns active provider configs", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.get(
        "http://127.0.0.1:5173/api/v1/ai/providers",
        { timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
