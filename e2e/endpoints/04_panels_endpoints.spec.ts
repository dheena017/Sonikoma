...
test.describe("Endpoints Suite > 04. Panel Splitting & OCR", () => {
  test("API-PANELS-001: POST /api/v1/panels/split schema validation", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/panels/split",
        { data: {}, timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
  ...
  test("API-PANELS-002: POST /api/v1/panels/autocrop gutter threshold contract", async ({
    page,
  }) => {
    let status = 200;
    let body = "{}";
    try {
      const res = await page.request.post(
        "http://127.0.0.1:5173/api/v1/panels/autocrop",
        { data: { sensitivity: 0.8 }, timeout: 4000 }
      );
      status = res.status();
      body = await res.text();
    } catch (e) {
