...
router.post("/generate", async (req, res) => {
  try {
    ...
    try {
      responsePanels = await generateDynamicPanels(parsed.title, parsed.genre, parsed.episode, scrapedUrls, model);
    } catch (err) {
      console.warn("Dynamic panels generation helper failed, using fallback:", err);
    }
