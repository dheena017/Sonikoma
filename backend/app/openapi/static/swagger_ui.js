...
(function () {
  ...
  if (originalFetch) {
    window.fetch = function (input, init) {
      var token =
        localStorage.getItem("sonikoma_jwt_token") ||
        sessionStorage.getItem("sonikoma_jwt_token");
      if (token) {
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  window.sonikomaFilterEndpoints = function (query) {
    if (
      window.ui &&
      window.ui.layoutActions &&
      typeof window.ui.layoutActions.updateFilter === "function"
    ) {
  ...
  window.sonikomaToggleAll = function () {
    ...
    var isAnyClosed = Array.from(opblocks).some(function (el) {
      return !el.classList.contains("is-open");
    });
    opblocks.forEach(function (block) {
      var summary = block.querySelector(".opblock-summary");
      if (summary) {
        if (isAnyClosed && !block.classList.contains("is-open"))
          summary.click();
        else if (!isAnyClosed && block.classList.contains("is-open"))
          summary.click();
      }
    });
  };
        ...
        if (
          ...
          if (window.ui) {
            ...
            if (
              ...
              typeof window.ui.authActions.authorize === "function"
            ) {
          ...
          statusEl.textContent =
            "Authorized successfully! Session is now active.";
          syncAuthStatus();
  ...
  var LOCKED_SVG_HTML =
    '<svg viewBox="0 0 20 20" width="14" height="14" fill="#10b981" style="display:block;margin:auto;"><path fill-rule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clip-rule="evenodd"/></svg>';
  var UNLOCKED_SVG_HTML =
    '<svg viewBox="0 0 20 20" width="14" height="14" fill="#818cf8" style="display:block;margin:auto;"><path d="M10 2a5 5 0 00-5 5v2a2 2 0 00-2 2v5a2 2 0 002 2h10a2 2 0 002-2v-5a2 2 0 00-2-2h-3V7a5 5 0 00-5-5zm0 2a3 3 0 013 3v2H7V7a3 3 0 013-3z"/></svg>';
  ...
  function syncAuthStatus() {
    ...
    if (token && window.ui) {
      try {
        ...
        if (
          ...
          typeof window.ui.authSelectors.authorized === "function"
        ) {
          var authObj = window.ui.authSelectors.authorized();
          if (authObj) {
            var jsAuth =
              typeof authObj.toJS === "function" ? authObj.toJS() : authObj;
    ...
    if (topBtn) {
      if (isAuthorized) {
        ...
        topBtn.innerHTML =
          '<span class="auth-icon">🔒</span><span>Authorized</span>';
      } else {
        topBtn.classList.remove("authorized");
        topBtn.innerHTML =
          '<span class="auth-icon">🔓</span><span>Authorize</span>';
      }
    }
    ...
    summaries.forEach(function (summary) {
      ...
      if (!tray) {
        ...
        tray.innerHTML = [
          ...
          '<button type="button" class="sonikoma-ctrl-btn sonikoma-lock-btn' +
            (isAuthorized ? " authorized" : "") +
            '" title="' +
            (isAuthorized ? "Authorized: JWT active" : "Click to Authorize") +
            '">' +
            (isAuthorized ? "🔒" : "🔓") +
            "</button>",
        ].join("");
        ...
        if (lockBtn) {
          if (isAuthorized && !lockBtn.classList.contains("authorized")) {
            ...
            !isAuthorized &&
            lockBtn.classList.contains("authorized")
          ) {
  ...
  function getCleanText(el) {
    ...
    var removeEls = clone.querySelectorAll(
      "button, .sonikoma-box-copy-btn, .download-contents, .download-contents-wrapper, .copy-to-clipboard, .sonikoma-arrow-btn, .sonikoma-copy-btn, .sonikoma-url-copy-btn"
    );
    removeEls.forEach(function (b) {
      b.remove();
    });
    var text = clone.innerText || clone.textContent || "";
    return text
      .replace(/^\s*(📋|✓|Copy|Copied!|Download|⬇|🪟|PowerShell)+\s*/gi, "")
      .replace(/\s*(📋|✓|Copy|Copied!|Download|⬇|🪟|PowerShell)+\s*$/gi, "")
      .trim();
  }
    ...
    if (!token) {
      try {
        if (
          ...
          if (authState) {
            var authObj =
              typeof authState.toJS === "function"
                ? authState.toJS()
                : authState;
            if (authObj.HTTPBearer && authObj.HTTPBearer.value) {
              var v = authObj.HTTPBearer.value;
              token =
                typeof v === "string" ? v : v.token || v.access_token || "";
            } else if (
              authObj.OAuth2PasswordBearer &&
              authObj.OAuth2PasswordBearer.value
            ) {
              ...
              token =
                typeof v2 === "string"
                  ? v2
                  : v2.access_token ||
                    (v2.token && v2.token.access_token) ||
                    v2.token ||
                    "";
            }
          }
      ...
      try {
        ...
        if (rawAuth) {
          ...
          if (parsed.HTTPBearer && parsed.HTTPBearer.value) {
            ...
            token =
              typeof v3 === "string" ? v3 : v3.token || v3.access_token || "";
          } else if (
            parsed.OAuth2PasswordBearer &&
            parsed.OAuth2PasswordBearer.value
          ) {
            ...
            token =
              typeof v4 === "string"
                ? v4
                : v4.access_token ||
                  (v4.token && v4.token.access_token) ||
                  v4.token ||
                  "";
          }
        }
  ...
  function injectAuthHeaderIntoCurl(curlText, token) {
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      if (
        !injected &&
        (lines[i].indexOf("http://") !== -1 ||
          lines[i].indexOf("https://") !== -1 ||
          i === 0)
      ) {
        ...
        result.push(
          "  -H 'Authorization: Bearer " + cleanToken + "' " + contChar
        );
        injected = true;
      }
    }
  ...
  function injectCodeCopyButtons() {
    function attachActionsToolbar(container, getTextFn, hasDownload, isCurl) {
      ...
      var parent =
        container.closest(".response-col_description") ||
        container.parentElement;
      if (parent) {
        var rogues = parent.querySelectorAll(
          ".download-contents, .download-contents-wrapper"
        );
      ...
      title.innerHTML = hasDownload
        ? "<span>📄</span><span>JSON Response</span>"
        : isCurl
        ? "<span>💻</span><span>cURL Command</span>"
        : "<span>⚡</span><span>Code Output</span>";
      ...
      if (isCurl) {
        ...
        psBtn.title =
          "Copy as Windows PowerShell (curl.exe) format with Authorization";
        ...
        psBtn.onclick = function (e) {
          ...
          navigator.clipboard
            ...
            .then(function () {
              psBtn.innerHTML =
                '<span class="icon">✓</span><span>Copied PS!</span>';
              psBtn.classList.add("copied");
              setTimeout(function () {
                psBtn.innerHTML =
                  '<span class="icon">🪟</span><span>PowerShell</span>';
    ...
    var curlPres = document.querySelectorAll(
      ".swagger-ui .curl pre, .swagger-ui .curl-command pre"
    );
    curlPres.forEach(function (el) {
      attachActionsToolbar(
        el,
        function (c) {
          return getCleanText(c);
        },
        false,
        true
      );
    ...
    var respCodes = document.querySelectorAll(
      ".swagger-ui .responses-table .highlight-code, .swagger-ui .responses-inner .highlight-code, .swagger-ui .response-col_description .highlight-code"
    );
    ...
    var headerPres = document.querySelectorAll(
      ".swagger-ui .response-headers pre, .swagger-ui .response-headers"
    );
