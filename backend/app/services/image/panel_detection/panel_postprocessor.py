"""
backend/app/services/image/panel_detector/postprocessor.py
─────────────────────────────────────────────────────────────────────────────
...
logger = logging.getLogger("sonikoma.services.image.panel_detection.panel_postprocessor")
...
def compute_overlap_count(boxes: List[Dict[str, Any]], iou_thresh: float = 0.40) -> int:
    ...
    for index, a in enumerate(normalized):
        ...
        for b in normalized[index + 1 :]:
            ...
            inter_x = max(0, min(a["x"] + a["w"], b["x"] + b["w"]) - max(a["x"], b["x"]))
            inter_h = max(0, min(a_y2, b["y"] + b["h"]) - max(a["y"], b["y"]))
            inter_area = inter_x * inter_h
            if inter_area <= 0:
                continue
...
def merge_similar_neighbor_slices(
    ...
    """Merges adjacent slices if similarity > 0.75 or if they share a dark background continuum."""
    ...
    for curr in boxes[1:]:
        ...
        if gap <= DEFAULT_MERGE_GAP:
            ...
            is_dark_gap = False
            if curr_y1 > prev_y2:
                gap_sample = gray_arr[prev_y2:curr_y1, :]
    ...
    for idx, (b, feat) in enumerate(zip(boxes, slice_features_list)):
        ...
        if bh < MIN_NOISE_HEIGHT or min_conf < 0.30:
            if resolved:
                ...
                new_x2 = max(p_x2, b_x2)
                new_y2 = max(p_y2, b_y2)

                prev["x"] = new_x1
                prev["y"] = new_y1
                prev["w"] = max(1, new_x2 - new_x1)
                prev["h"] = max(1, new_y2 - new_y1)
