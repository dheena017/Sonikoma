...
Features:
  - Draw YOLO detections with masks and bounding boxes
  - Export multi-stage diagnostic pipeline visualization
  - Generate comprehensive quality reports and statistics
...
def _extract_box_coords(obj: Union[Dict[str, Any], Any]) -> Tuple[int, int, int, int]:
    """
    Safely extract bounding box coordinates (x, y, width, height) from either
    dict or object with attributes. Returns (x, y, w, h) or (0, 0, 0, 0) if invalid.
    
    Args:
        obj: Dictionary or object with coordinate properties.
        
    Returns:
        Tuple of (x, y, width, height) coordinates as integers.
    """
    try:
...
def _get_depth(obj: Union[Dict[str, Any], Any], default: int = 0) -> int:
    """Get depth property from object or dict."""
    try:
        if isinstance(obj, dict):
...
def _get_confidence(obj: Union[Dict[str, Any], Any], default: float = 0.90) -> float:
    """Get confidence property from object or dict."""
    try:
        if isinstance(obj, dict):
...
def draw_yolo_detections(image_path: str, conf_threshold: float = 0.25,
    ...
    Args:
        ...
        colors: ColorScheme instance for visualization (default: DEFAULT_COLORS).
        config: VisualizationConfig instance (default: DEFAULT_CONFIG).
        
    Returns:
        PNG-encoded bytes of the annotated image, or None if YOLO model is unavailable.
    ...
    try:
        ...
        # Handle various return types from YOLO model
        if results is None:
        ...
        # Validate that r has the expected attributes
            ...
            try:
                ...
                for mask_tensor in mask_data:
                    try:
                        ...
                        # Skip scalar/1D masks (invalid)
                        if mask_np.ndim < 2:
                            logger.debug("[DebugViz] Skipping scalar/1D mask")
                            continue
        ...
        if boxes is not None:
            try:
                # Convert boxes to iterable list if needed
...
def _draw_banner(frame: np.ndarray, text: str, 
    ...
    """Draw banner with text at the top of the frame."""
    if colors is None:
        colors = DEFAULT_COLORS
    if config is None:
        config = DEFAULT_CONFIG
...
def _add_no_detection_banner(frame: np.ndarray, 
    ...
    """Add 'no detections' banner to frame."""
...
def export_multi_stage_debug_images(
    ...
    """
    Exports full-resolution 10-artifact diagnostic pipeline output:
      01_original.png: Full-res input canvas
      02_separator_heatmap.png: Full-res row score visualization (Green=gutter, Red=content, Yellow=uncertain)
      03_separator_rows.png: Cut lines and gutter bands
      04_candidate_boxes.png: Raw initial candidate boxes
      05_after_merge.png: Post-merge candidate boxes
      06_after_recursive_split.png: Post-recursive split candidate boxes
      07_final_boxes.png: Final panel bounding boxes with tags
      08_crop_preview.png: Downscaled preview grid of final crops
      09_pipeline_summary.json: Execution metadata, timings & confidence scores
      10_panel_lineage.json: Detailed merge/split provenance records
      
    Args:
        image_src: Image source (file path, numpy array, or PIL Image).
        panel_bounds: List of panel bounding boxes/objects.
        output_dir: Output directory (default: data/debug_output).
        job_id: Optional job identifier for file prefixes.
        separator_scores: Array of separator row scores.
        separator_bands: List of separator row Y-coordinates.
        raw_candidates: Raw candidate boxes before merging.
        merged_candidates: Candidate boxes after merging.
        pipeline_summary: Custom pipeline summary data.
        lineage_tracker: Panel lineage/provenance data.
        colors: ColorScheme instance (default: DEFAULT_COLORS).
        config: VisualizationConfig instance (default: DEFAULT_CONFIG).
        enable_timing: Whether to track and log execution timing.
    ...
    try:
        ...
        # Legacy dictionary keys for backward compatibility
        if p01:
...
# ============================================================================
# HELPER FUNCTIONS FOR EXPORT PIPELINE
# ============================================================================
...
def _load_image(image_src: Any, job_id: Optional[str] = None) -> Optional[Image.Image]:
    ...
    Args:
        image_src: File path (str), numpy array, or PIL Image.
...
def _generate_separator_heatmap(frame: np.ndarray, separator_scores: Optional[np.ndarray],
    ...
    """Generate and save separator heatmap visualization."""
    try:
...
def _generate_separator_rows(frame: np.ndarray, separator_bands: Optional[List[int]],
                             ...
                             output_dir: str, prefix: str) -> Optional[str]:
    """Generate and save separator rows visualization."""
    try:
...
def _generate_candidate_boxes(frame: np.ndarray, candidates: List[Any],
    ...
    """Generate and save candidate boxes visualization."""
    try:
...
def _generate_merged_boxes(frame: np.ndarray, merged: List[Any],
                          colors: ColorScheme, output_dir: str, prefix: str) -> Optional[str]:
    """Generate and save merged boxes visualization."""
    try:
...
def _generate_split_boxes(frame: np.ndarray, panel_bounds: List[Any],
                         colors: ColorScheme, output_dir: str, prefix: str) -> Optional[str]:
    """Generate and save recursive split visualization."""
    try:
...
def _generate_final_boxes(frame: np.ndarray, panel_bounds: List[Any],
                         ...
                         output_dir: str, prefix: str) -> Optional[str]:
    """Generate and save final boxes with labels."""
    try:
...
def _generate_crop_preview(pil_img: Image.Image, panel_bounds: List[Any],
                          ...
                          output_dir: str, prefix: str) -> Optional[str]:
    """Generate and save crop preview grid."""
    try:
        w, h = pil_img.size
        cropped_imgs = []
...
def _generate_pipeline_summary(panel_bounds: List[Any], w: int, h: int,
                              ...
                              output_dir: str, prefix: str) -> Optional[str]:
    """Generate and save pipeline summary JSON."""
    try:
        panel_heights = []
        confidences = []
...
def _generate_panel_lineage(panel_bounds: List[Any],
    ...
    """Generate and save panel lineage JSON."""
    try:
...
def batch_draw_yolo_detections(image_paths: List[str], 
    ...
    """
    Process multiple images with YOLO detection drawing.
    ...
    Returns:
        Dictionary mapping image paths to PNG bytes (or None for failures).
    """
    results = {}
...
def get_quality_metrics(panel_bounds: List[Any]) -> Dict[str, Any]:
    """
    Calculate quality metrics for panel detection results.
    
    Args:
        panel_bounds: List of detected panel bounds.
...
def cleanup_debug_output(output_dir: Optional[str] = None, 
                        max_age_days: int = 7,
    ...
    Args:
        output_dir: Debug output directory (default: data/debug_output).
        max_age_days: Maximum age of files to keep in days.
        dry_run: If True, don't actually delete files.
        
    Returns:
        Dictionary with counts of analyzed and deleted files.
    """
    if output_dir is None or output_dir == "debug_output":
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "..", ".."))
        output_dir = os.path.join(project_root, "data", "debug_output")
    ...
    if not os.path.isdir(output_dir):
        logger.warning(f"[DebugViz] Debug output directory not found: {output_dir}")
        return {"analyzed": 0, "deleted": 0}
                ...
                if file_age > max_age_seconds:
                    if not dry_run:
                        try:
                            os.remove(filepath)
                            stats["deleted"] += 1
                            logger.debug(f"[DebugViz] Deleted old debug file: {filename}")
                        except Exception as e:
                            logger.warning(f"[DebugViz] Failed to delete {filename}: {e}")
                    else:
                        stats["deleted"] += 1
                        logger.debug(f"[DebugViz] (DRY RUN) Would delete: {filename}")
    except Exception as e:
        logger.error(f"[DebugViz] Cleanup error: {e}")

    return stats


def export_quality_report(panel_bounds: List[Any], output_path: str) -> bool:
    """
    Export detailed quality report to a file.
    ...
    """
    try:
        metrics = get_quality_metrics(panel_bounds)
        report = {
        ...
        logger.info(f"[DebugViz] Quality report exported to {output_path}")
        return True
    except Exception as e:
        logger.error(f"[DebugViz] Failed to export quality report: {e}")
        return False
