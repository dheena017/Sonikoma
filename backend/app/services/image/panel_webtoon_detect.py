...
def _classify_content_rows(
    ...
    if ocr_boxes:
        for box in ocr_boxes:
            ...
            if bh_b >= int(slice_h * 0.70):
                continue
            if not (bx + bw_b < col_x1 or bx > col_x2):
                by1_rel = max(0, by - start_y)
                by2_rel = min(slice_h, (by + bh_b) - start_y)
                if by2_rel > by1_rel:
                    ocr_sub = center_slice[by1_rel:by2_rel]
                    if ocr_sub.size > 0:
                        ocr_bg = (ocr_sub > threshold_val) if is_white_bg else (ocr_sub < threshold_val)
                        ocr_bg_ratio = np.sum(ocr_bg, axis=1) / float(center_w)
                        valid_ocr_rows = (ocr_bg_ratio < 0.95)
                        content_rows[by1_rel:by2_rel] |= valid_ocr_rows
...
def _detect_panels_webtoon(
    ...
    # ── Gutter Detection Tuning Parameters ─────────────────────────────────────
    # ── Gutter Detection Tuning Parameters ─────────────────────────────────────
    ...
    if ocr_boxes:
        bubble_pad = max(2, min(40, int(h * 0.04)))
        max_ocr_h = min(500, max(100, int(h * 0.10)))
        for box in ocr_boxes:
            bh_b = int(box.get("h", 0))
            if bh_b >= max_ocr_h:
                continue
            ...
            if by2 > by1:
                for r in range(by1, by2):
                    if stroke_counts[r] >= 1 or bg_ratio[r] < 0.95:
                        is_content_row[r] = True
        ...
        for col_x1, col_x2 in col_ranges:
            ...
            if slice_h > 40:
                ...
                if not np.any(c_rows):
                    logger.info(f"[Panel Webtoon Detect] Skipping empty gutter slice [{start_y} -> {end_y}]")
                    continue
                ...
                if calc_h < min_height_px:
                    logger.info(f"[Panel Webtoon Detect] Skipping sub-min-height trimmed slice [{start_y} -> {end_y}] ({calc_h}px < {min_height_px}px)")
                    continue
    ...
    if len(raw_boxes) > 1:
        ...
        for i in range(len(raw_boxes)):
            ...
            if i + 1 < len(raw_boxes):
                ...
                is_curr_bubble = curr["h"] < 220
                is_nxt_bubble = nxt["h"] < 220
                ...
                detected_bubble = bool(curr.get("bubble_candidate") or nxt.get("bubble_candidate"))

                if (
                    gap_y <= 150 and
                    detected_bubble and
                    not separator_in_gap and
                    same_x_center and
                    x_overlap_ratio >= 0.50 and
                    (is_curr_bubble or is_nxt_bubble)
                ):
                    ...
                    logger.info(
                        f"[Panel Webtoon Detect] Unifying speech bubble slice at y={curr['y']} with panel at y={nxt['y']} (gap={gap_y}px)"
                    )
