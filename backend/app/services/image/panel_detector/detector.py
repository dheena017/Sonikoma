...
from app.services.image.panel_detector.postprocessor import compute_post_panel_confidence
from app.services.image.utils.panel_box_utils import (
    PanelBounds,
    adjust_to_aspect_ratio,
    merge_overlapping_boxes
)
from app.services.image.panel_detector.webtoon_detect import (
    _detect_bg_color_and_threshold,
    _detect_panels_webtoon
)
from app.services.image.panel_detector.grid_detect import (
...
logger = logging.getLogger("sonikoma.services.image.panel_detector.detector")
...
def detect_panels_in_image(
    ...
    if _run_ocr_for_protection:
        try:
            import asyncio
            from app.services.image.ocr import extract_full_ocr_data
            try:
    ...
    if use_yolo:
        try:
            from app.services.image.panel_detector.yolo_speech_bubble_detector import get_yolo_speech_bubble_model
    ...
    for high_sensitivity in passes:
        ...
        from app.services.image.panel_detector.postprocessor import (
    ...
    try:
        from app.services.image.panel_detector.debug_visualizer import export_multi_stage_debug_images
        debug_pb_list = [
