"""
backend/app/services/image/scraper/scraper.py
─────────────────────────────────────────────────────────────────────────────
...
from services.image.scraper.url_utils import extract_webtoon_url
from services.image.scraper.parsers import (
    USER_AGENTS,
    UNWANTED_PATTERNS,
    parse_episode_index,
...
from services.image.scraper.client import (
...
logger = logging.getLogger("sonikoma.services.image.scraper")
...
async def scrape_images_from_url(
    ...
    """
    Crawls a Webtoon episode page and isolates the panel image URLs in memory.
    Processes HTML payload in-memory without saving diagnostic files to disk unless save_debug_html is True.
    """
    ...
    save_html_env = os.getenv("SAVE_SCRAPED_HTML", "false").lower() in ("true", "1", "yes")
    if html and (save_debug_html or save_html_env):
        try:
