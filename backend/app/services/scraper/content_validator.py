...
from .scraper_models import CandidateImage, ImageItem, ImageSourceType
...
class ImageValidator:
    ...
    def validate_candidates(
        ...
        for idx, cand in enumerate(candidates):
            raw_url = cand.get("url") if isinstance(cand, dict) else getattr(cand, "url", None)
            if not raw_url:
                continue
            ...
            if not url.startswith(("http://", "https://", "data:image/", "/api/v1/proxy/image")):
            ...
            is_inside = cand.get("is_inside_reader", True) if isinstance(cand, dict) else getattr(cand, "is_inside_reader", True)
            confidence = (cand.get("confidence", 1.0) if isinstance(cand, dict) else getattr(cand, "confidence", 1.0)) or 1.0
            cand_width = cand.get("width") if isinstance(cand, dict) else getattr(cand, "width", None)
            cand_height = cand.get("height") if isinstance(cand, dict) else getattr(cand, "height", None)
            source_val = cand.get("source_type", ImageSourceType.DOM) if isinstance(cand, dict) else getattr(cand, "source_type", ImageSourceType.DOM)
            ...
            if cand_width is not None and cand_width < DEFAULT_MIN_IMAGE_WIDTH:
                ...
                continue
            if cand_height is not None and cand_height < DEFAULT_MIN_IMAGE_HEIGHT:
