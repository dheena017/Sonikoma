...
logger = logging.getLogger('sonikoma.infrastructure.storage')
...
def cleanup_cached_url(url: Optional[str]) -> None:
    ...
    if 'data/media/' in url:
        try:
            ...
            logger.error(f"[Storage] Failed to delete file {url}: {e}")
...
def delete_video_file(video_url: Optional[str]) -> None:
    ...
    try:
        ...
        logger.error(f"[Storage] Failed to delete video file {video_url}: {e}")
