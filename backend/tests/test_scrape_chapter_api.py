...
MOCK_USER = {"user_id": "test_user_123", "email": "test@example.com"}
...
def setup_db_and_auth():
    ...
    try:
        conn.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?)",
            ("test_user_123", "testuser", "test@example.com", "hash", "2025-01-01", "2025-01-01")
        )
        conn.commit()
    finally:
        conn.close()
...
def test_scrape_chapter_unauthenticated_returns_401():
    app.dependency_overrides.clear()
    response = client.post("/api/v1/scraper/chapter", json={"url": "https://example.com"})
    assert response.status_code == 401


def test_scrape_chapter_empty_url_returns_400():
    response = client.post("/api/v1/scraper/chapter", json={"url": ""})
...
def test_get_job_status_api():
    job = job_manager.create_job(job_type=JobType.PANEL_SPLIT, user_id="test_user_123", project_id="p1")
...
def test_cancel_job_api():
    job = job_manager.create_job(job_type=JobType.OCR, user_id="test_user_123", project_id="p2")
